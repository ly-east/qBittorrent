...
namespace {
template <typename T> int threeWayCompare(const T &left, const T &right) {
  if (left == right)
    return 0;
...
int customCompare(
    const TagSet &left, const TagSet &right,
    const Utils::Compare::NaturalCompare<Qt::CaseInsensitive> &compare) {
...
template <typename T> int customCompare(const T left, const T right) {
...
TransferListSortModel::TransferListSortModel(QObject *parent)
    : QSortFilterProxyModel{parent},
      m_subSortColumn{u"TransferList/SubSortColumn"_s,
                      TransferListModel::TR_NAME, adjustSubSortColumn},
...
void TransferListSortModel::setTrackerFilter(
...
int TransferListSortModel::compare(const QModelIndex &left,
  ...
  case TransferListModel::TR_INFOHASH_V1:
    return threeWayCompare(leftValue.value<SHA1Hash>(),
                           rightValue.value<SHA1Hash>());

  case TransferListModel::TR_INFOHASH_V2:
    return threeWayCompare(leftValue.value<SHA256Hash>(),
                           rightValue.value<SHA256Hash>());

  case TransferListModel::TR_TAGS:
    return customCompare(leftValue.value<TagSet>(), rightValue.value<TagSet>(),
                         m_naturalCompare);
  ...
  case TransferListModel::TR_SEEDS: {
    ...
    const auto totalL =
        left.data(TransferListModel::AdditionalUnderlyingDataRole).toInt();
    const auto totalR =
        right.data(TransferListModel::AdditionalUnderlyingDataRole).toInt();
...
bool TransferListSortModel::lessThan(const QModelIndex &left,
  ...
  if (result == 0) {
    const int subResult = compare(left.sibling(left.row(), m_subSortColumn),
                                  right.sibling(right.row(), m_subSortColumn));
    // Qt inverses lessThan() result when ordered descending.
    // For sub-sorting we have to do it manually.
    // When both are ordered descending subResult must be double-inversed, which
    // is the same as no inversion.
    const bool inverseSubResult =
        (m_lastSortOrder != m_subSortOrder); // exactly one is descending
...
bool TransferListSortModel::matchFilter(const int sourceRow,
  ...
  const BitTorrent::Torrent *torrent =
      model->torrentHandle(model->index(sourceRow, 0, sourceParent));
  if (!torrent)
    return false;
