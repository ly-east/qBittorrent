...
SpeedLimitDialog::SpeedLimitDialog(QWidget *parent)
    : QDialog{parent}, m_ui{new Ui::SpeedLimitDialog},
  ...
  m_ui->labelGlobalSpeedIcon->setPixmap(
      UIThemeManager::instance()->getScaledPixmap(
  ...
  const auto initSlider = [](QSlider *slider, const int value,
                             const int maximum) {
  ...
  const int downloadVal =
      std::max(0, (session->globalDownloadSpeedLimit() / 1024));
  const int maxUpload =
      std::max(10000, (session->globalUploadSpeedLimit() / 1024));
  const int maxDownload =
      std::max(10000, (session->globalDownloadSpeedLimit() / 1024));
  initSlider(m_ui->sliderUploadLimit, uploadVal, maxUpload);
  initSlider(m_ui->sliderDownloadLimit, downloadVal, maxDownload);

  const int altUploadVal =
      std::max(0, (session->altGlobalUploadSpeedLimit() / 1024));
  const int altDownloadVal =
      std::max(0, (session->altGlobalDownloadSpeedLimit() / 1024));
  const int altMaxUpload =
      std::max(10000, (session->altGlobalUploadSpeedLimit() / 1024));
  const int altMaxDownload =
      std::max(10000, (session->altGlobalDownloadSpeedLimit() / 1024));
  ...
  m_initialValues = {
      m_ui->spinUploadLimit->value(), m_ui->spinDownloadLimit->value(),
      m_ui->spinAltUploadLimit->value(), m_ui->spinAltDownloadLimit->value()};
  ...
  connect(m_ui->sliderAltUploadLimit, &QSlider::valueChanged,
          m_ui->spinAltUploadLimit, &QSpinBox::setValue);
  connect(m_ui->sliderAltDownloadLimit, &QSlider::valueChanged,
          m_ui->spinAltDownloadLimit, &QSpinBox::setValue);
  connect(m_ui->spinAltUploadLimit, qOverload<int>(&QSpinBox::valueChanged),
          this, [this](const int value) {
            updateSliderValue(m_ui->sliderAltUploadLimit, value);
          });
  connect(m_ui->spinAltDownloadLimit, qOverload<int>(&QSpinBox::valueChanged),
          this, [this](const int value) {
            updateSliderValue(m_ui->sliderAltDownloadLimit, value);
          });
...
void SpeedLimitDialog::accept() {
  ...
  if (m_initialValues.altDownloadSpeedLimit !=
