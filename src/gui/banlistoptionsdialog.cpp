...
BanListOptionsDialog::BanListOptionsDialog(QWidget *parent)
    : QDialog(parent), m_ui(new Ui::BanListOptionsDialog),
      m_storeDialogSize(SETTINGS_KEY(u"Size"_s)),
      m_model(new QStringListModel(BitTorrent::Session::instance()->bannedIPs(),
                                   this)) {
  m_ui->setupUi(this);
...
void BanListOptionsDialog::on_buttonBanIP_clicked() {
  ...
  if (!Utils::Net::isValidIP(ip)) {
    QMessageBox::warning(this, tr("Warning"),
                         tr("The entered IP address is invalid."));
    return;
  }
  ...
  for (int i = 0; i < m_sortFilter->rowCount(); ++i) {
    ...
    if (ip == index.data().toString()) {
      QMessageBox::warning(this, tr("Warning"),
                           tr("The entered IP is already banned."));
      return;
    }
  }
...
void BanListOptionsDialog::on_buttonDeleteIP_clicked() {
  QModelIndexList selection =
      m_ui->bannedIPList->selectionModel()->selectedIndexes();
