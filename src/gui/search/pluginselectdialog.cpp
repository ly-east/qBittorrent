...
PluginSelectDialog::PluginSelectDialog(SearchPluginManager *pluginManager,
                                       QWidget *parent)
    : QDialog(parent), m_ui(new Ui::PluginSelectDialog),
  ...
  m_ui->actionUninstall->setIcon(
      UIThemeManager::instance()->getIcon(u"list-remove"_s));

  connect(m_ui->actionEnable, &QAction::toggled, this,
          &PluginSelectDialog::enableSelection);
  ...
  connect(m_ui->pluginsTree, &QTreeWidget::itemDoubleClicked, this,
          &PluginSelectDialog::togglePluginState);

  loadSupportedSearchPlugins();

  connect(m_pluginManager, &SearchPluginManager::pluginInstalled, this,
          &PluginSelectDialog::pluginInstalled);
  connect(m_pluginManager, &SearchPluginManager::pluginInstallationFailed, this,
          &PluginSelectDialog::pluginInstallationFailed);
  connect(m_pluginManager, &SearchPluginManager::pluginUpdated, this,
          &PluginSelectDialog::pluginUpdated);
  connect(m_pluginManager, &SearchPluginManager::pluginUpdateFailed, this,
          &PluginSelectDialog::pluginUpdateFailed);
  connect(m_pluginManager, &SearchPluginManager::checkForUpdatesFinished, this,
          &PluginSelectDialog::checkForUpdatesFinished);
  connect(m_pluginManager, &SearchPluginManager::checkForUpdatesFailed, this,
          &PluginSelectDialog::checkForUpdatesFailed);
...
void PluginSelectDialog::on_closeButton_clicked() { close(); }
...
void PluginSelectDialog::on_actionUninstall_triggered() {
  ...
  if (error)
    QMessageBox::warning(
        this, tr("Uninstall warning"),
        tr("Some plugins could not be uninstalled because they are included in "
           "qBittorrent. Only the ones you added yourself can be "
           "uninstalled.\nThose plugins were disabled."));
  else
    QMessageBox::information(
        this, tr("Uninstall success"),
        tr("All selected plugins were uninstalled successfully"));
}
...
PluginSelectDialog::findItemsWithUrl(const QString &url) {
...
void PluginSelectDialog::addNewPlugin(const QString &pluginName) {
  ...
  if (plugin->iconPath.exists()) {
    ...
    item->setData(PLUGIN_NAME, Qt::DecorationRole,
                  QIcon(plugin->iconPath.data()));
  } else {
...
void PluginSelectDialog::finishPluginUpdate() {
  ...
  if ((m_pendingUpdates == 0) && !m_updatedPlugins.isEmpty()) {
    ...
    QMessageBox::information(this, tr("Search plugin update"),
                             tr("Plugins installed or updated: %1")
                                 .arg(m_updatedPlugins.join(u", ")));
...
void PluginSelectDialog::on_installButton_clicked() {
  ...
  connect(dlg, &PluginSourceDialog::askForLocalFile, this,
          &PluginSelectDialog::askForLocalPlugin);
  connect(dlg, &PluginSourceDialog::askForUrl, this,
          &PluginSelectDialog::askForPluginUrl);
  dlg->show();
}
...
void PluginSelectDialog::askForPluginUrl() {
  ...
  if (Net::DownloadManager::hasSupportedScheme(clipTxt) &&
      clipTxt.endsWith(u".py"))
    defaultUrl = clipTxt;
  QString url = AutoExpandableDialog::getText(
      this, tr("New search engine plugin URL"), tr("URL:"), QLineEdit::Normal,
      defaultUrl, &ok);

  while (ok && !url.isEmpty() && !url.endsWith(u".py")) {
    QMessageBox::warning(
        this, tr("Invalid link"),
        tr("The link doesn't seem to point to a search engine plugin."));
    url =
        AutoExpandableDialog::getText(this, tr("New search engine plugin URL"),
...
void PluginSelectDialog::iconDownloadFinished(
    const Net::DownloadResult &result) {
  if (result.status != Net::DownloadStatus::Success) {
    qDebug("Could not download favicon: %s, reason: %s",
           qUtf8Printable(result.url), qUtf8Printable(result.errorString));
    return;
  }
  ...
  if (!invalid) {
    for (QTreeWidgetItem *item : asConst(findItemsWithUrl(result.url))) {
      ...
      const QString ext = result.url.endsWith(u".ico", Qt::CaseInsensitive)
                              ? u".ico"_s
                              : u".png"_s;
      const Path iconPath =
          SearchPluginManager::pluginsLocation() / Path(id + ext);
      if (Utils::Fs::copyFile(filePath, iconPath)) {
        ...
        bool invalidExt = (sizesExt.isEmpty() ||
                           iconWithExt.pixmap(sizesExt.first()).isNull());
        if (invalidExt) {
...
void PluginSelectDialog::checkForUpdatesFinished(
    const QHash<QString, PluginVersion> &updateInfo) {
  finishAsyncOp();
  if (updateInfo.isEmpty()) {
    QMessageBox::information(this, tr("Search plugin update"),
                             tr("All your plugins are already up to date."));
    return;
  }
...
void PluginSelectDialog::checkForUpdatesFailed(const QString &reason) {
  finishAsyncOp();
  QMessageBox::warning(
      this, tr("Search plugin update"),
      tr("Sorry, couldn't check for plugin updates. %1").arg(reason));
}
