...
SearchJobWidget::SearchJobWidget(SearchHandler *searchHandler, QWidget *parent)
    : QWidget(parent), m_ui(new Ui::SearchJobWidget),
  ...
  m_searchListModel =
      new QStandardItemModel(0, SearchSortModel::NB_SEARCH_COLUMNS, this);
  m_searchListModel->setHeaderData(SearchSortModel::NAME, Qt::Horizontal,
                                   tr("Name", "i.e: file name"));
  m_searchListModel->setHeaderData(SearchSortModel::SIZE, Qt::Horizontal,
                                   tr("Size", "i.e: file size"));
  m_searchListModel->setHeaderData(
      SearchSortModel::SEEDS, Qt::Horizontal,
      tr("Seeders", "i.e: Number of full sources"));
  m_searchListModel->setHeaderData(
      SearchSortModel::LEECHES, Qt::Horizontal,
      tr("Leechers", "i.e: Number of partial sources"));
  m_searchListModel->setHeaderData(SearchSortModel::ENGINE_URL, Qt::Horizontal,
                                   tr("Search engine"));
  // Set columns text alignment
  m_searchListModel->setHeaderData(SearchSortModel::SIZE, Qt::Horizontal,
                                   QVariant(Qt::AlignRight | Qt::AlignVCenter),
                                   Qt::TextAlignmentRole);
  m_searchListModel->setHeaderData(SearchSortModel::SEEDS, Qt::Horizontal,
                                   QVariant(Qt::AlignRight | Qt::AlignVCenter),
                                   Qt::TextAlignmentRole);
  m_searchListModel->setHeaderData(SearchSortModel::LEECHES, Qt::Horizontal,
                                   QVariant(Qt::AlignRight | Qt::AlignVCenter),
                                   Qt::TextAlignmentRole);
  ...
  for (int i = 0; i < SearchSortModel::DL_LINK; ++i) {
    if ((m_ui->resultsBrowser->columnWidth(i) <= 0) &&
  ...
  connect(header(), &QWidget::customContextMenuRequested, this,
          &SearchJobWidget::displayColumnHeaderMenu);
  ...
  m_lineEditSearchResultsFilter->setPlaceholderText(
      tr("Filter search results..."));
  m_lineEditSearchResultsFilter->setContextMenuPolicy(Qt::CustomContextMenu);
  connect(m_lineEditSearchResultsFilter, &QWidget::customContextMenuRequested,
          this, &SearchJobWidget::showFilterContextMenu);
  connect(m_lineEditSearchResultsFilter, &LineEdit::textChanged, this,
  ...
  connect(m_ui->minSeeds, &QAbstractSpinBox::editingFinished, this,
          &SearchJobWidget::updateFilter);
  connect(m_ui->minSeeds, qOverload<int>(&QSpinBox::valueChanged), this,
          &SearchJobWidget::updateFilter);
  connect(m_ui->maxSeeds, &QAbstractSpinBox::editingFinished, this,
          &SearchJobWidget::updateFilter);
  connect(m_ui->maxSeeds, qOverload<int>(&QSpinBox::valueChanged), this,
          &SearchJobWidget::updateFilter);
  connect(m_ui->minSize, &QAbstractSpinBox::editingFinished, this,
          &SearchJobWidget::updateFilter);
  connect(m_ui->minSize, qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &SearchJobWidget::updateFilter);
  connect(m_ui->maxSize, &QAbstractSpinBox::editingFinished, this,
          &SearchJobWidget::updateFilter);
  connect(m_ui->maxSize, qOverload<double>(&QDoubleSpinBox::valueChanged), this,
          &SearchJobWidget::updateFilter);
  ...
  connect(m_ui->resultsBrowser, &QAbstractItemView::doubleClicked, this,
          &SearchJobWidget::onItemDoubleClicked);

  connect(searchHandler, &SearchHandler::newSearchResults, this,
          &SearchJobWidget::appendSearchResults);
  connect(searchHandler, &SearchHandler::searchFinished, this,
          &SearchJobWidget::searchFinished);
  connect(searchHandler, &SearchHandler::searchFailed, this,
...
void SearchJobWidget::setRowColor(int row, const QColor &color) {
  ...
  for (int i = 0; i < m_proxyModel->columnCount(); ++i)
    m_proxyModel->setData(m_proxyModel->index(row, i), color,
                          Qt::ForegroundRole);

  m_proxyModel->setDynamicSortFilter(true);
}

SearchJobWidget::Status SearchJobWidget::status() const { return m_status; }
...
void SearchJobWidget::cancelSearch() { m_searchHandler->cancelSearch(); }
...
void SearchJobWidget::openTorrentPages() const {
  const QModelIndexList rows{
      m_ui->resultsBrowser->selectionModel()->selectedRows()};
  for (const QModelIndex &rowIndex : rows) {
    const QString descrLink =
        m_proxyModel
            ->data(
                m_proxyModel->index(rowIndex.row(), SearchSortModel::DESC_LINK))
            .toString();
...
void SearchJobWidget::copyField(const int column) const {
  ...
  for (const QModelIndex &rowIndex : rows) {
    const QString field =
        m_proxyModel->data(m_proxyModel->index(rowIndex.row(), column))
            .toString();
    if (!field.isEmpty())
      list << field;
  }
...
void SearchJobWidget::downloadTorrent(const QModelIndex &rowIndex,
  ...
  const QString torrentUrl =
      m_proxyModel
          ->data(m_proxyModel->index(rowIndex.row(), SearchSortModel::DL_LINK))
          .toString();
  const QString siteUrl = m_proxyModel
                              ->data(m_proxyModel->index(
                                  rowIndex.row(), SearchSortModel::ENGINE_URL))
                              .toString();

  if (torrentUrl.startsWith(u"magnet:", Qt::CaseInsensitive)) {
    addTorrentToSession(torrentUrl, option);
  } else {
    SearchDownloadHandler *downloadHandler =
        m_searchHandler->manager()->downloadTorrent(siteUrl, torrentUrl);
    connect(downloadHandler, &SearchDownloadHandler::downloadFinished, this,
            [this, option](const QString &source) {
              addTorrentToSession(source, option);
            });
    connect(downloadHandler, &SearchDownloadHandler::downloadFinished,
            downloadHandler, &SearchDownloadHandler::deleteLater);
  }
  setRowColor(rowIndex.row(),
              QApplication::palette().color(QPalette::LinkVisited));
}
...
void SearchJobWidget::addTorrentToSession(const QString &source,
  ...
  if ((option == AddTorrentOption::ShowDialog) ||
      ((option == AddTorrentOption::Default) &&
...
void SearchJobWidget::updateResultsCount() {
  ...
  m_ui->resultsLbl->setText(
      tr("Results (showing <i>%1</i> out of <i>%2</i>):", "i.e: Search results")
          .arg(filteredResults)
          .arg(totalResults));
...
void SearchJobWidget::updateFilter() {
  ...
  m_proxyModel->enableNameFilter(filteringMode() ==
                                 NameFilteringMode::OnlyNames);
  // we update size and seeds filter parameters in the model even if they are
  // disabled
  m_proxyModel->setSeedsFilter(m_ui->minSeeds->value(),
                               m_ui->maxSeeds->value());
  m_proxyModel->setSizeFilter(
      sizeInBytes(m_ui->minSize->value(),
                  static_cast<SizeUnit>(m_ui->minSizeUnit->currentIndex())),
      sizeInBytes(m_ui->maxSize->value(),
                  static_cast<SizeUnit>(m_ui->maxSizeUnit->currentIndex())));
...
void SearchJobWidget::fillFilterComboBoxes() {
  ...
  m_ui->filterMode->addItem(tr("Torrent names only"),
                            static_cast<int>(NameFilteringMode::OnlyNames));
  m_ui->filterMode->addItem(tr("Everywhere"),
                            static_cast<int>(NameFilteringMode::Everywhere));

  const QVariant selectedMode =
      static_cast<int>(m_nameFilteringMode.get(NameFilteringMode::OnlyNames));
...
void SearchJobWidget::showFilterContextMenu() {
  ...
  connect(useRegexAct, &QAction::toggled, this, [this]() {
    filterSearchResults(m_lineEditSearchResultsFilter->text());
  });
...
void SearchJobWidget::contextMenuEvent(QContextMenuEvent *event) {
  ...
  menu->addSeparator();
  menu->addAction(UIThemeManager::instance()->getIcon(u"application-url"_s),
                  tr("Open description page"), this,
                  &SearchJobWidget::openTorrentPages);

  QMenu *copySubMenu = menu->addMenu(
      UIThemeManager::instance()->getIcon(u"edit-copy"_s), tr("Copy"));

  copySubMenu->addAction(
      UIThemeManager::instance()->getIcon(u"name"_s, u"edit-copy"_s),
      tr("Name"), this, &SearchJobWidget::copyTorrentNames);
  copySubMenu->addAction(
      UIThemeManager::instance()->getIcon(u"insert-link"_s, u"edit-copy"_s),
      tr("Download link"), this, &SearchJobWidget::copyTorrentDownloadLinks);
  copySubMenu->addAction(
      UIThemeManager::instance()->getIcon(u"application-url"_s, u"edit-copy"_s),
      tr("Description page URL"), this, &SearchJobWidget::copyTorrentURLs);
...
SearchJobWidget::NameFilteringMode SearchJobWidget::filteringMode() const {
  return static_cast<NameFilteringMode>(
      m_ui->filterMode->itemData(m_ui->filterMode->currentIndex()).toInt());
}
...
int SearchJobWidget::visibleColumnsCount() const {
  int count = 0;
  for (int i = 0, iMax = m_ui->resultsBrowser->header()->count(); i < iMax;
       ++i) {
    if (!m_ui->resultsBrowser->isColumnHidden(i))
      ++count;
  }
...
void SearchJobWidget::displayColumnHeaderMenu() {
  ...
  for (int i = 0; i < SearchSortModel::DL_LINK; ++i) {
    const auto columnName =
        m_searchListModel->headerData(i, Qt::Horizontal, Qt::DisplayRole)
            .toString();
    QAction *action =
  ...
  QAction *resizeAction = menu->addAction(tr("Resize columns"), this, [this]() {
    for (int i = 0, count = m_ui->resultsBrowser->header()->count(); i < count;
         ++i) {
      if (!m_ui->resultsBrowser->isColumnHidden(i))
        m_ui->resultsBrowser->resizeColumnToContents(i);
    }
...
void SearchJobWidget::searchFailed() { setStatus(Status::Error); }

void SearchJobWidget::appendSearchResults(
  ...
  for (const SearchResult &result : results) {
    ...
    const auto setModelData =
        [this, row](const int column, const QString &displayData,
          ...
          m_searchListModel->setItemData(m_searchListModel->index(row, column),
                                         data);
        };
    ...
    setModelData(SearchSortModel::DESC_LINK, result.descrLink,
                 result.descrLink);
    setModelData(SearchSortModel::SIZE,
                 Utils::Misc::friendlyUnit(result.fileSize), result.fileSize,
                 (Qt::AlignRight | Qt::AlignVCenter));
    setModelData(SearchSortModel::SEEDS, QString::number(result.nbSeeders),
                 result.nbSeeders, (Qt::AlignRight | Qt::AlignVCenter));
    setModelData(SearchSortModel::LEECHES, QString::number(result.nbLeechers),
                 result.nbLeechers, (Qt::AlignRight | Qt::AlignVCenter));
  }
