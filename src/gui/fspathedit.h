...
class FileSystemPathEdit : public QWidget {
  ...
  Q_PROPERTY(Path selectedPath READ selectedPath WRITE setSelectedPath NOTIFY
                 selectedPathChanged)
...
public:
  ...
  enum class Mode {
    ...
    ReadOnly //!< no browse button and no dialog, only validate path and check
             //!< read permission
  };
  Q_ENUM(Mode)
...
protected:
  explicit FileSystemPathEdit(Private::IFileEditorWithCompletion *editor,
                              QWidget *parent);

  template <class Widget> Widget *editWidget() const {
