...
QPixmap Utils::Gui::scaledPixmap(const QIcon &icon, const QWidget *widget,
                                 const int height) {
...
QPixmap Utils::Gui::scaledPixmap(const Path &path, const QWidget *widget,
                                 const int height) {
  Q_UNUSED(widget);
  Q_ASSERT(height >= 0);
  ...
  return (height == 0)
             ? pixmap
             : pixmap.scaledToHeight(height, Qt::SmoothTransformation);
}
...
QSize Utils::Gui::smallIconSize(const QWidget *widget) {
  ...
  const int s = QApplication::style()->pixelMetric(QStyle::PM_SmallIconSize,
                                                   nullptr, widget);
  return {s, s};
}
...
QSize Utils::Gui::largeIconSize(const QWidget *widget) {
  ...
  const int s = QApplication::style()->pixelMetric(QStyle::PM_LargeIconSize,
                                                   nullptr, widget);
  return {s, s};
}

QPoint Utils::Gui::screenCenter(const QWidget *w) {
  // Returns the QPoint which the widget will be placed center on screen (where
  // parent resides)
  ...
  const QPoint primaryScreenCenter{
      (r.x() + (r.width() - w->frameSize().width()) / 2),
      (r.y() + (r.height() - w->frameSize().height()) / 2)};
  ...
  return {(r.x() + (r.width() - w->frameSize().width()) / 2),
          (r.y() + (r.height() - w->frameSize().height()) / 2)};
}
...
void Utils::Gui::openPath(const Path &path) {
  ...
  auto *thread = QThread::create([path]() {
    if (SUCCEEDED(::CoInitializeEx(
      ...
      ::ShellExecuteW(nullptr, nullptr, pathWStr.c_str(), nullptr, nullptr,
                      SW_SHOWNORMAL);
...
void Utils::Gui::openFolderSelect(const Path &path) {
  ...
  const auto output =
      QString::fromLocal8Bit(proc.readLine(lineMaxLength).simplified());
  if ((output == u"dolphin.desktop") ||
      (output == u"org.kde.dolphin.desktop")) {
    proc.startDetached(u"dolphin"_s, {u"--select"_s, path.toString()});
  } else if ((output == u"nautilus.desktop") ||
             (output == u"org.gnome.Nautilus.desktop") ||
             (output == u"nautilus-folder-handler.desktop")) {
    proc.start(u"nautilus"_s, {u"--version"_s});
    proc.waitForFinished();
    const auto nautilusVerStr =
        QString::fromLocal8Bit(proc.readLine(lineMaxLength))
            .remove(QRegularExpression(u"[^0-9.]"_s));
    using NautilusVersion = Utils::Version<3>;
    if (NautilusVersion::fromString(nautilusVerStr, {1, 0, 0}) >
        NautilusVersion(3, 28, 0))
      proc.startDetached(
          u"nautilus"_s,
          {(Fs::isDir(path) ? path.parentPath() : path).toString()});
    else
      proc.startDetached(
          u"nautilus"_s,
          {u"--no-desktop"_s,
    ...
    proc.startDetached(
        u"nemo"_s, {u"--no-desktop"_s,
                    (Fs::isDir(path) ? path.parentPath() : path).toString()});
  } else if ((output == u"konqueror.desktop") ||
             (output == u"kfmclient_dir.desktop")) {
    proc.startDetached(u"konqueror"_s, {u"--select"_s, path.toString()});
  } else {
    // "caja" manager can't pinpoint the file, see:
    // https://github.com/qbittorrent/qBittorrent/issues/5003
    openPath(path.parentPath());
  }
#else
  openPath(path.parentPath());
