...
CookiesModel::CookiesModel(const QList<QNetworkCookie> &cookies,
                           QObject *parent)
    : QAbstractItemModel(parent), m_cookies(cookies) {}

QList<QNetworkCookie> CookiesModel::cookies() const { return m_cookies; }

QVariant CookiesModel::headerData(int section, Qt::Orientation orientation,
                                  int role) const {
...
QModelIndex CookiesModel::index(int row, int column,
                                const QModelIndex &parent) const {
  if (parent.isValid() // no items with valid parent
      || (row < 0) || (row >= m_cookies.size()) || (column < 0) ||
      (column >= NB_COLUMNS))
    return {};
...
bool CookiesModel::setData(const QModelIndex &index, const QVariant &value,
                           int role) {
  if (role != Qt::EditRole)
    return false;
...
bool CookiesModel::removeRows(int row, int count, const QModelIndex &parent) {
  if ((m_cookies.isEmpty()) || (row >= m_cookies.size()) ||
