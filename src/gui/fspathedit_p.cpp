...
bool Private::FileSystemPathValidator::filesOnly() const { return m_filesOnly; }
...
Private::FileSystemPathValidator::testPath(const Path &path) const {
  // `QFileInfo` will cache the query results and avoid excessive querying to
  // filesystem
...
QValidator::State Private::FileSystemPathValidator::validate(QString &input,
  ...
  m_lastValidationState =
      (m_lastTestResult == TestResult::OK)
...
Private::FileLineEdit::FileLineEdit(QWidget *parent) : QLineEdit(parent) {
...
Private::FileLineEdit::~FileLineEdit() {
  delete m_completerModel; // has to be deleted before deleting the
                           // m_iconProvider object
  delete m_iconProvider;
}

void Private::FileLineEdit::completeDirectoriesOnly(
    const bool completeDirsOnly) {
...
QWidget *Private::FileLineEdit::widget() { return this; }
...
void Private::FileLineEdit::validateText() {
  ...
  const FileSystemPathValidator::TestResult lastTestResult =
      validator->lastTestResult();
...
QString Private::FileLineEdit::warningText(
    const FileSystemPathValidator::TestResult result) {
...
Private::FileComboEdit::FileComboEdit(QWidget *parent) : QComboBox{parent} {
  setEditable(true);
  setLineEdit(new FileLineEdit(this));
}

void Private::FileComboEdit::completeDirectoriesOnly(bool completeDirsOnly) {
  static_cast<FileLineEdit *>(lineEdit())
      ->completeDirectoriesOnly(completeDirsOnly);
}
...
QWidget *Private::FileComboEdit::widget() { return this; }

QString Private::FileComboEdit::text() const { return currentText(); }
