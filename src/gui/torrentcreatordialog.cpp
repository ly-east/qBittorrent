...
namespace {
// When the root file/directory of the created torrent is a symlink, we want to
// keep the symlink name in the torrent. On Windows, however,
// QFileDialog::DontResolveSymlinks disables shortcuts (.lnk files) expansion,
// making it impossible to pick a file if its path contains a shortcut. As of
// NTFS symlinks, they don't seem to be resolved anyways.
#ifndef Q_OS_WIN
...
TorrentCreatorDialog::TorrentCreatorDialog(QWidget *parent,
                                           const Path &defaultPath)
    : QDialog(parent), m_ui(new Ui::TorrentCreatorDialog),
  ...
  connect(m_ui->addFileButton, &QPushButton::clicked, this,
          &TorrentCreatorDialog::onAddFileButtonClicked);
  connect(m_ui->addFolderButton, &QPushButton::clicked, this,
          &TorrentCreatorDialog::onAddFolderButtonClicked);
  connect(m_ui->buttonBox, &QDialogButtonBox::accepted, this,
          &TorrentCreatorDialog::onCreateButtonClicked);
  connect(m_ui->buttonBox, &QDialogButtonBox::rejected, this, &QDialog::reject);
  connect(m_ui->buttonCalcTotalPieces, &QPushButton::clicked, this,
          &TorrentCreatorDialog::updatePiecesCount);
  connect(m_ui->checkStartSeeding, &QCheckBox::clicked,
          m_ui->checkIgnoreShareLimits, &QWidget::setEnabled);

  connect(m_creatorThread, &BitTorrent::TorrentCreatorThread::creationSuccess,
          this, &TorrentCreatorDialog::handleCreationSuccess);
  connect(m_creatorThread, &BitTorrent::TorrentCreatorThread::creationFailure,
          this, &TorrentCreatorDialog::handleCreationFailure);
  connect(m_creatorThread, &BitTorrent::TorrentCreatorThread::updateProgress,
...
void TorrentCreatorDialog::onAddFolderButtonClicked() {
  ...
  const Path path{QFileDialog::getExistingDirectory(
      this, tr("Select folder"), oldPath,
...
int TorrentCreatorDialog::getPieceSize() const {
  const int pieceSizes[] = {
      0,    16,   32,   64,   128,   256,  512,
      1024, 2048, 4096, 8192, 16384, 32768}; // base unit in KiB
...
void TorrentCreatorDialog::onCreateButtonClicked() {
#ifdef Q_OS_WIN
  // Resolve the path in case it contains a shortcut (otherwise, the following
  // usages will consider it invalid)
  const Path inputPath =
      Utils::Fs::toCanonicalPath(m_ui->textInputPath->selectedPath());
#else
  ...
  if (!Utils::Fs::isReadable(inputPath)) {
    QMessageBox::critical(this, tr("Torrent creation failed"),
                          tr("Reason: Path to file/folder is not readable."));
    return;
  }

  // get save path
  const Path lastSavePath = (m_storeLastSavePath.get(Utils::Fs::homePath()) /
                             Path(inputPath.filename() + u".torrent"));
  Path destPath{QFileDialog::getSaveFileName(
      this, tr("Select where to save the new torrent"), lastSavePath.data(),
...
void TorrentCreatorDialog::handleCreationFailure(const QString &msg) {
  ...
  QMessageBox::information(this, tr("Torrent creation failed"),
                           tr("Reason: %1").arg(msg));
  setInteractionEnabled(true);
}

void TorrentCreatorDialog::handleCreationSuccess(const Path &path,
  ...
  if (m_ui->checkStartSeeding->isChecked()) {
    ...
    if (!result) {
      QMessageBox::critical(this, tr("Torrent creation failed"),
                            tr("Reason: Created torrent is invalid. It won't "
                               "be added to download list."));
      return;
    }
    ...
    if (m_ui->checkIgnoreShareLimits->isChecked()) {
      ...
      params.inactiveSeedingTimeLimit =
          BitTorrent::Torrent::NO_INACTIVE_SEEDING_TIME_LIMIT;
    }
    params.useAutoTMM = false; // otherwise if it is on by default, it will
                               // overwrite `savePath` to the default save path

    BitTorrent::Session::instance()->addTorrent(result.value(), params);
  }
  QMessageBox::information(
      this, tr("Torrent creator"),
...
void TorrentCreatorDialog::updatePiecesCount() {
  ...
  const int count = BitTorrent::TorrentCreatorThread::calculateTotalPieces(
      path, getPieceSize(), isAlignmentOptimized, getPaddedFileSizeLimit());
#endif
...
void TorrentCreatorDialog::setInteractionEnabled(const bool enabled) const {
  ...
  m_ui->checkIgnoreShareLimits->setEnabled(
      enabled && m_ui->checkStartSeeding->isChecked());
#ifdef QBT_USES_LIBTORRENT2
...
void TorrentCreatorDialog::loadSettings() {
  m_ui->textInputPath->setSelectedPath(
      m_storeLastAddPath.get(Utils::Fs::homePath()));
