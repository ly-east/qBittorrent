...
HtmlBrowser::HtmlBrowser(QWidget *parent) : QTextBrowser(parent) {
  ...
  m_diskCache->setCacheDirectory(
      (specialFolderLocation(SpecialFolder::Cache) / Path(u"rss"_s)).data());
  m_diskCache->setMaximumCacheSize(50 * 1024 * 1024);
  qDebug() << "HtmlBrowser  cache path:" << m_diskCache->cacheDirectory()
           << " max size:" << m_diskCache->maximumCacheSize() / 1024 / 1024
           << "MB";
  m_netManager->setCache(m_diskCache);

  connect(m_netManager, &QNetworkAccessManager::finished, this,
          &HtmlBrowser::resourceLoaded);
}
...
void HtmlBrowser::resourceLoaded(QNetworkReply *reply) {
  ...
  if ((reply->error() == QNetworkReply::NoError) && (reply->size() > 0)) {
    qDebug() << "HtmlBrowser::resourceLoaded() save "
             << reply->request().url().toString();
  } else {
    // If resource failed to load, replace it with warning icon and store it in
    // cache for 1 day. Otherwise HTMLBrowser will keep trying to download it
    // every time article is displayed, since it's not possible to cache error
    // responses.
