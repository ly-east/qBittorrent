...
ArticleListWidget::ArticleListWidget(QWidget *parent) : QListWidget(parent) {
...
QListWidgetItem *
ArticleListWidget::mapRSSArticle(RSS::Article *rssArticle) const {
...
void ArticleListWidget::setRSSItem(RSS::Item *rssItem, bool unreadOnly) {
  ...
  m_rssItem = rssItem;
  if (m_rssItem) {
    connect(m_rssItem, &RSS::Item::newArticle, this,
            &ArticleListWidget::handleArticleAdded);
    connect(m_rssItem, &RSS::Item::articleRead, this,
            &ArticleListWidget::handleArticleRead);
    connect(m_rssItem, &RSS::Item::articleAboutToBeRemoved, this,
            &ArticleListWidget::handleArticleAboutToBeRemoved);
...
void ArticleListWidget::handleArticleRead(RSS::Article *rssArticle) {
  ...
  const QBrush foregroundBrush{
      UIThemeManager::instance()->getColor(u"RSS.ReadArticle"_s)};
  item->setData(Qt::ForegroundRole, foregroundBrush);
  item->setData(Qt::DecorationRole, UIThemeManager::instance()->getIcon(
                                        u"rss_read_article"_s, u"sphere"_s));

  checkInvariant();
}

void ArticleListWidget::handleArticleAboutToBeRemoved(
    RSS::Article *rssArticle) {
...
QListWidgetItem *ArticleListWidget::createItem(RSS::Article *article) const {
  ...
  if (article->isRead()) {
    ...
    const QBrush foregroundBrush{
        UIThemeManager::instance()->getColor(u"RSS.UnreadArticle"_s)};
    item->setData(Qt::ForegroundRole, foregroundBrush);
    item->setData(Qt::DecorationRole,
                  UIThemeManager::instance()->getIcon(u"rss_unread_article"_s,
                                                      u"sphere"_s));
  }
