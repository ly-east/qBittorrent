...
QIcon rssFeedIcon(const RSS::Feed *feed) {
  ...
  if (feed->hasError())
    return UIThemeManager::instance()->getIcon(u"task-reject"_s,
                                               u"unavailable"_s);
...
FeedListWidget::FeedListWidget(QWidget *parent) : QTreeWidget(parent) {
  ...
  connect(RSS::Session::instance(), &RSS::Session::itemAdded, this,
          &FeedListWidget::handleItemAdded);
  connect(RSS::Session::instance(), &RSS::Session::feedStateChanged, this,
          &FeedListWidget::handleFeedStateChanged);
  connect(RSS::Session::instance(), &RSS::Session::feedIconLoaded, this,
          &FeedListWidget::handleFeedIconLoaded);
  connect(RSS::Session::instance(), &RSS::Session::itemPathChanged, this,
          &FeedListWidget::handleItemPathChanged);
  connect(RSS::Session::instance(), &RSS::Session::itemAboutToBeRemoved, this,
          &FeedListWidget::handleItemAboutToBeRemoved);

  m_rssToTreeItemMapping[RSS::Session::instance()->rootFolder()] =
      invisibleRootItem();

  m_unreadStickyItem = new FeedListItem(this);
  m_unreadStickyItem->setData(0, Qt::UserRole,
                              QVariant::fromValue(reinterpret_cast<intptr_t>(
                                  RSS::Session::instance()->rootFolder())));
  m_unreadStickyItem->setText(
      0, tr("Unread  (%1)")
             .arg(RSS::Session::instance()->rootFolder()->unreadCount()));
  m_unreadStickyItem->setData(
      0, Qt::DecorationRole,
      UIThemeManager::instance()->getIcon(u"mail-inbox"_s));
...
void FeedListWidget::handleItemAdded(RSS::Item *rssItem) {
  auto *parentItem =
      m_rssToTreeItemMapping.value(RSS::Session::instance()->itemByPath(
          RSS::Item::parentPath(rssItem->path())));
...
void FeedListWidget::handleItemUnreadCountChanged(RSS::Item *rssItem) {
  if (rssItem == RSS::Session::instance()->rootFolder()) {
    ...
    item->setData(0, Qt::DisplayRole,
                  u"%1  (%2)"_s.arg(rssItem->name(),
                                    QString::number(rssItem->unreadCount())));
  }
}
...
void FeedListWidget::handleItemPathChanged(RSS::Item *rssItem) {
  ...
  RSS::Item *parentRssItem = RSS::Session::instance()->itemByPath(
      RSS::Item::parentPath(rssItem->path()));
...
FeedListWidget::getAllOpenedFolders(QTreeWidgetItem *parent) const {
...
RSS::Item *FeedListWidget::getRSSItem(QTreeWidgetItem *item) const {
  ...
  return reinterpret_cast<RSS::Item *>(
      item->data(0, Qt::UserRole).value<intptr_t>());
}
...
void FeedListWidget::dragMoveEvent(QDragMoveEvent *event) {
  ...
  if ((item ==
       m_unreadStickyItem) // Prohibit dropping onto global unread counter
      || selectedItems().contains(
             m_unreadStickyItem) // Prohibit dragging of global unread counter
...
void FeedListWidget::dropEvent(QDropEvent *event) {
  ...
  RSS::Folder *destFolder =
      (destFolderItem ? static_cast<RSS::Folder *>(getRSSItem(destFolderItem))
                      : RSS::Session::instance()->rootFolder());
  ...
  for (QTreeWidgetItem *srcItem : asConst(selectedItems())) {
    ...
    RSS::Session::instance()->moveItem(
        rssItem, RSS::Item::joinPath(destFolder->path(), rssItem->name()));
  }
...
QTreeWidgetItem *FeedListWidget::createItem(RSS::Item *rssItem,
  ...
  item->setData(0, Qt::UserRole,
                QVariant::fromValue(reinterpret_cast<intptr_t>(rssItem)));
  ...
  connect(rssItem, &RSS::Item::unreadCountChanged, this,
          &FeedListWidget::handleItemUnreadCountChanged);
