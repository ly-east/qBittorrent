...
IPSubnetWhitelistOptionsDialog::IPSubnetWhitelistOptionsDialog(QWidget *parent)
    : QDialog(parent), m_ui(new Ui::IPSubnetWhitelistOptionsDialog),
  ...
  for (const Utils::Net::Subnet &subnet :
       asConst(Preferences::instance()->getWebUIAuthSubnetWhitelist()))
...
void IPSubnetWhitelistOptionsDialog::on_buttonWhitelistIPSubnet_clicked() {
  const std::optional<Utils::Net::Subnet> subnet =
      Utils::Net::parseSubnet(m_ui->txtIPSubnet->text());
  if (!subnet) {
    QMessageBox::critical(this, tr("Error"),
                          tr("The entered subnet is invalid."));
    return;
  }

  m_model->insertRow(m_model->rowCount());
  m_model->setData(m_model->index(m_model->rowCount() - 1, 0),
                   Utils::Net::subnetToString(subnet.value()));
  m_ui->txtIPSubnet->clear();
  m_modified = true;
}

void IPSubnetWhitelistOptionsDialog::on_buttonDeleteIPSubnet_clicked() {
  for (const auto &i : asConst(
           m_ui->whitelistedIPSubnetList->selectionModel()->selectedIndexes()))
    m_sortFilter->removeRow(i.row());

  m_modified = true;
}

void IPSubnetWhitelistOptionsDialog::on_txtIPSubnet_textChanged(
    const QString &subnetStr) {
  m_ui->buttonWhitelistIPSubnet->setEnabled(
      Utils::Net::parseSubnet(subnetStr).has_value());
}
