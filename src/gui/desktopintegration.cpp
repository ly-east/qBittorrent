...
#define NOTIFICATIONS_SETTINGS_KEY(name)                                       \
  (SETTINGS_KEY(u"Notifications/"_s) name)
...
#endif
{
  ...
  if (isNotificationsEnabled()) {
    ...
    connect(m_notifier, &DBusNotifier::messageClicked, this,
            &DesktopIntegration::notificationClicked);
  }
#endif
#endif

  connect(Preferences::instance(), &Preferences::changed, this,
          &DesktopIntegration::onPreferencesChanged);
}

DesktopIntegration::~DesktopIntegration() { delete m_menu; }
...
QString DesktopIntegration::toolTip() const { return m_toolTip; }
...
QMenu *DesktopIntegration::menu() const { return m_menu; }
...
void DesktopIntegration::showNotification(const QString &title,
  ...
  if (m_systrayIcon && QSystemTrayIcon::supportsMessages())
    m_systrayIcon->showMessage(title, msg, QSystemTrayIcon::Information,
                               notificationTimeout());
#endif
#endif
}
