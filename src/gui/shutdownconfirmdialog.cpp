...
ShutdownConfirmDialog::ShutdownConfirmDialog(QWidget *parent,
                                             const ShutdownDialogAction &action)
  ...
  connect(&m_timer, &QTimer::timeout, this,
          &ShutdownConfirmDialog::updateSeconds);
}

ShutdownConfirmDialog::~ShutdownConfirmDialog() { delete m_ui; }
...
bool ShutdownConfirmDialog::askForConfirmation(
    QWidget *parent, const ShutdownDialogAction &action) {
...
void ShutdownConfirmDialog::accept() {
  Preferences::instance()->setDontConfirmAutoExit(
      m_ui->neverShowAgainCheckbox->isChecked());
  QDialog::accept();
}
...
void ShutdownConfirmDialog::updateText() {
  QString t = tr("You can cancel the action within %1 seconds.")
                  .arg(QString::number(m_timeout)) +
              u'\n';
