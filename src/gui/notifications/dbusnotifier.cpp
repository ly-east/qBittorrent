...
DBusNotifier::DBusNotifier(QObject *parent)
    : QObject(parent), m_notificationsInterface{new DBusNotificationsInterface(
  ...
  connect(m_notificationsInterface, &DBusNotificationsInterface::ActionInvoked,
          this, &DBusNotifier::onActionInvoked);
...
void DBusNotifier::showMessage(const QString &title, const QString &message,
  ...
  const QVariantMap hints{
      {u"desktop-entry"_s, u"org.qbittorrent.qBittorrent"_s}};
  const QDBusPendingReply<uint> reply =
      m_notificationsInterface->notify(u"qBittorrent"_s, 0, u"qbittorrent"_s,
...
void DBusNotifier::onActionInvoked(const uint messageID,
...
void DBusNotifier::onNotificationClosed(const uint messageID,
                                        const uint reason) {
  Q_UNUSED(reason);
