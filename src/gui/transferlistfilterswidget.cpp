...
private:
  void paintEvent(QPaintEvent *) override {
    ...
    indicatorOption.rect = style()->subElementRect(QStyle::SE_CheckBoxIndicator,
                                                   &indicatorOption, this);
    indicatorOption.state |=
        (QStyle::State_Children |
         (isChecked() ? QStyle::State_Open : QStyle::State_None));
    style()->drawPrimitive(QStyle::PE_IndicatorBranch, &indicatorOption,
                           &painter, this);

    QStyleOptionButton labelOption;
    initStyleOption(&labelOption);
    labelOption.rect = style()->subElementRect(QStyle::SE_CheckBoxContents,
                                               &labelOption, this);
    style()->drawControl(QStyle::CE_CheckBoxLabel, &labelOption, &painter,
                         this);
  }
};
} // namespace
...
TransferListFiltersWidget::TransferListFiltersWidget(
    QWidget *parent, TransferListWidget *transferList,
  ...
  connect(statusLabel, &QCheckBox::toggled, pref,
          &Preferences::setStatusFilterState);
  mainWidgetLayout->addWidget(statusLabel);

  auto *statusFilters = new StatusFilterWidget(this, transferList);
  connect(statusLabel, &QCheckBox::toggled, statusFilters,
          &StatusFilterWidget::toggleFilter);
  ...
  connect(m_categoryFilterWidget,
          &CategoryFilterWidget::actionDeleteTorrentsTriggered, transferList,
          &TransferListWidget::deleteVisibleTorrents);
  connect(m_categoryFilterWidget,
          &CategoryFilterWidget::actionPauseTorrentsTriggered, transferList,
          &TransferListWidget::pauseVisibleTorrents);
  connect(m_categoryFilterWidget,
          &CategoryFilterWidget::actionResumeTorrentsTriggered, transferList,
  ...
  connect(tagsLabel, &QCheckBox::toggled, this,
          &TransferListFiltersWidget::onTagFilterStateChanged);
  ...
  connect(m_tagFilterWidget, &TagFilterWidget::tagChanged, transferList,
  ...
  connect(trackerLabel, &QCheckBox::toggled, pref,
          &Preferences::setTrackerFilterState);
  mainWidgetLayout->addWidget(trackerLabel);

  m_trackersFilterWidget =
      new TrackersFilterWidget(this, transferList, downloadFavicon);
  connect(trackerLabel, &QCheckBox::toggled, m_trackersFilterWidget,
          &TrackersFilterWidget::toggleFilter);
...
void TransferListFiltersWidget::removeTrackers(
    const BitTorrent::Torrent *torrent, const QStringList &trackers) {
  m_trackersFilterWidget->removeTrackers(torrent, trackers);
}

void TransferListFiltersWidget::refreshTrackers(
    const BitTorrent::Torrent *torrent) {
  m_trackersFilterWidget->refreshTrackers(torrent);
}

void TransferListFiltersWidget::changeTrackerless(
    const BitTorrent::Torrent *torrent, const bool trackerless) {
  m_trackersFilterWidget->changeTrackerless(torrent, trackerless);
}

void TransferListFiltersWidget::trackerEntriesUpdated(
    const BitTorrent::Torrent *torrent,
    const QHash<QString, BitTorrent::TrackerEntry> &updatedTrackerEntries) {
  m_trackersFilterWidget->handleTrackerEntriesUpdated(torrent,
...
void TransferListFiltersWidget::toggleCategoryFilter(bool enabled) {
  ...
  m_transferList->applyCategoryFilter(
      enabled ? m_categoryFilterWidget->currentCategory() : QString());
}
...
void TransferListFiltersWidget::toggleTagFilter(bool enabled) {
  ...
  m_transferList->applyTagFilter(enabled ? m_tagFilterWidget->currentTag()
                                         : QString());
}
