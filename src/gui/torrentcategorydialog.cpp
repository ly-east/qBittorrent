...
TorrentCategoryDialog::TorrentCategoryDialog(QWidget *parent)
  ...
  connect(m_ui->textCategoryName, &QLineEdit::textChanged, this,
          &TorrentCategoryDialog::categoryNameChanged);
  connect(m_ui->comboUseDownloadPath, &QComboBox::currentIndexChanged, this,
          &TorrentCategoryDialog::useDownloadPathChanged);
}

TorrentCategoryDialog::~TorrentCategoryDialog() { delete m_ui; }

QString
TorrentCategoryDialog::createCategory(QWidget *parent,
                                      const QString &parentCategoryName) {
  ...
  while (dialog.exec() == TorrentCategoryDialog::Accepted) {
    ...
    if (!Session::isValidCategoryName(newCategoryName)) {
      QMessageBox::critical(parent, tr("Invalid category name"),
...
void TorrentCategoryDialog::editCategory(QWidget *parent,
  ...
  dialog->setCategoryOptions(
      Session::instance()->categoryOptions(categoryName));
  connect(dialog, &TorrentCategoryDialog::accepted, parent,
          [dialog, categoryName]() {
            Session::instance()->editCategory(categoryName,
                                              dialog->categoryOptions());
          });
  dialog->open();
}
...
void TorrentCategoryDialog::setCategoryName(const QString &categoryName) {
  ...
  m_ui->textCategoryName->setSelection(
      subcategoryNameStart, (categoryName.size() - subcategoryNameStart));
}
...
BitTorrent::CategoryOptions TorrentCategoryDialog::categoryOptions() const {
  ...
  if (m_ui->comboUseDownloadPath->currentIndex() == 1)
    categoryOptions.downloadPath = {true,
                                    m_ui->comboDownloadPath->selectedPath()};
...
void TorrentCategoryDialog::setCategoryOptions(
    const BitTorrent::CategoryOptions &categoryOptions) {
  m_ui->comboSavePath->setSelectedPath(categoryOptions.savePath);
  if (categoryOptions.downloadPath) {
    m_ui->comboUseDownloadPath->setCurrentIndex(
        categoryOptions.downloadPath->enabled ? 1 : 2);
    m_ui->comboDownloadPath->setSelectedPath(
        categoryOptions.downloadPath->enabled
            ? categoryOptions.downloadPath->path
            : Path());
  } else {
...
void TorrentCategoryDialog::categoryNameChanged(const QString &categoryName) {
  ...
  const bool useDownloadPath =
      (index == 1) ||
      ((index == 0) &&
       BitTorrent::Session::instance()->isDownloadPathEnabled());
  if (useDownloadPath)
    m_ui->comboDownloadPath->setPlaceholder(btSession->downloadPath() /
                                            categoryPath);

  m_ui->buttonBox->button(QDialogButtonBox::Ok)
      ->setEnabled(!categoryName.isEmpty());
}

void TorrentCategoryDialog::useDownloadPathChanged(const int index) {
  if (const Path selectedPath = m_ui->comboDownloadPath->selectedPath();
  ...
  m_ui->comboDownloadPath->setSelectedPath(
      (index == 1) ? m_lastEnteredDownloadPath : Path());

  const QString categoryName = m_ui->textCategoryName->text();
  const Path categoryPath = BitTorrent::Session::instance()->downloadPath() /
                            Utils::Fs::toValidPath(categoryName);
  const bool useDownloadPath =
      (index == 1) ||
      ((index == 0) &&
       BitTorrent::Session::instance()->isDownloadPathEnabled());
  m_ui->comboDownloadPath->setPlaceholder(useDownloadPath ? categoryPath
                                                          : Path());
}
