...
AutoExpandableDialog::~AutoExpandableDialog() { delete m_ui; }
...
void AutoExpandableDialog::showEvent(QShowEvent *e) {
  // Overriding showEvent is required for consistent UI with fixed size under
  // custom DPI
  QDialog::showEvent(e);

  // Show dialog and resize textbox to fit the text
  // NOTE: For unknown reason QFontMetrics gets more accurate when called from
  // showEvent.
  int wd =
      m_ui->textEdit->fontMetrics().horizontalAdvance(m_ui->textEdit->text()) +
      4;
  ...
  if (!m_ui->textLabel->text().isEmpty()) {
    int w = m_ui->textLabel->fontMetrics().horizontalAdvance(
        m_ui->textLabel->text());
    wd = std::max(wd, w);
  }
  ...
  if (wd > width()) {
    QSize size = {width() - m_ui->verticalLayout->sizeHint().width() + wd,
                  height()};
    resize(size);
  }
}
