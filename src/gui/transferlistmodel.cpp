...
QHash<BitTorrent::TorrentState, QColor> torrentStateColorsFromUITheme() {
  ...
  const TorrentStateColorDescriptor colorDescriptors[] = {
      ...
      {BitTorrent::TorrentState::StalledDownloading,
       u"TransferList.StalledDownloading"_s},
      {BitTorrent::TorrentState::DownloadingMetadata,
       u"TransferList.DownloadingMetadata"_s},
      {BitTorrent::TorrentState::ForcedDownloadingMetadata,
       u"TransferList.ForcedDownloadingMetadata"_s},
      {BitTorrent::TorrentState::ForcedDownloading,
       u"TransferList.ForcedDownloading"_s},
      {BitTorrent::TorrentState::Uploading, u"TransferList.Uploading"_s},
      {BitTorrent::TorrentState::StalledUploading,
       u"TransferList.StalledUploading"_s},
      {BitTorrent::TorrentState::ForcedUploading,
       u"TransferList.ForcedUploading"_s},
      {BitTorrent::TorrentState::QueuedDownloading,
       u"TransferList.QueuedDownloading"_s},
      {BitTorrent::TorrentState::QueuedUploading,
       u"TransferList.QueuedUploading"_s},
      {BitTorrent::TorrentState::CheckingDownloading,
       u"TransferList.CheckingDownloading"_s},
      {BitTorrent::TorrentState::CheckingUploading,
       u"TransferList.CheckingUploading"_s},
      {BitTorrent::TorrentState::CheckingResumeData,
       u"TransferList.CheckingResumeData"_s},
      {BitTorrent::TorrentState::PausedDownloading,
       u"TransferList.PausedDownloading"_s},
      {BitTorrent::TorrentState::PausedUploading,
       u"TransferList.PausedUploading"_s},
  ...
  for (const TorrentStateColorDescriptor &colorDescriptor : colorDescriptors) {
    const QColor themeColor =
        UIThemeManager::instance()->getColor(colorDescriptor.id);
...
TransferListModel::TransferListModel(QObject *parent)
    : QAbstractListModel{parent},
      m_statusStrings{
          ...
          {BitTorrent::TorrentState::ForcedDownloadingMetadata,
           tr("[F] Downloading metadata",
              "Used when forced to load a magnet link. You probably shouldn't "
              "translate the F.")},
          {BitTorrent::TorrentState::ForcedDownloading,
           tr("[F] Downloading", "Used when the torrent is forced started. You "
                                 "probably shouldn't translate the F.")},
          {BitTorrent::TorrentState::Uploading,
           tr("Seeding", "Torrent is complete and in upload-only mode")},
          {BitTorrent::TorrentState::StalledUploading,
           tr("Seeding", "Torrent is complete and in upload-only mode")},
          {BitTorrent::TorrentState::ForcedUploading,
           tr("[F] Seeding", "Used when the torrent is forced started. You "
                             "probably shouldn't translate the F.")},
          {BitTorrent::TorrentState::QueuedDownloading,
          ...
          {BitTorrent::TorrentState::CheckingUploading,
           tr("Checking", "Torrent local data is being checked")},
          {BitTorrent::TorrentState::CheckingResumeData,
           tr("Checking resume data",
              "Used when loading the torrents from disk after qbt is launched. "
              "It checks the correctness of the .fastresume file. Normally it "
              "is completed in a fraction of a second, unless loading many "
              "many torrents.")},
          {BitTorrent::TorrentState::PausedDownloading, tr("Paused")},
          {BitTorrent::TorrentState::PausedUploading, tr("Completed")},
          {BitTorrent::TorrentState::Moving,
      ...
      m_checkingIcon{UIThemeManager::instance()->getIcon(u"force-recheck"_s,
                                                         u"checking"_s)},
      m_completedIcon{UIThemeManager::instance()->getIcon(
          u"checked-completed"_s, u"completed"_s)},
      ...
      m_pausedIcon{UIThemeManager::instance()->getIcon(
          u"stopped"_s, u"media-playback-pause"_s)},
      ...
      m_uploadingIcon{
          UIThemeManager::instance()->getIcon(u"upload"_s, u"uploading"_s)} {
  configure();
  connect(Preferences::instance(), &Preferences::changed, this,
          &TransferListModel::configure);
  ...
  connect(Session::instance(), &Session::torrentsLoaded, this,
          &TransferListModel::addTorrents);
  connect(Session::instance(), &Session::torrentAboutToBeRemoved, this,
          &TransferListModel::handleTorrentAboutToBeRemoved);
  connect(Session::instance(), &Session::torrentsUpdated, this,
          &TransferListModel::handleTorrentsUpdated);

  connect(Session::instance(), &Session::torrentFinished, this,
          &TransferListModel::handleTorrentStatusUpdated);
  connect(Session::instance(), &Session::torrentMetadataReceived, this,
          &TransferListModel::handleTorrentStatusUpdated);
  connect(Session::instance(), &Session::torrentResumed, this,
          &TransferListModel::handleTorrentStatusUpdated);
  connect(Session::instance(), &Session::torrentPaused, this,
          &TransferListModel::handleTorrentStatusUpdated);
  connect(Session::instance(), &Session::torrentFinishedChecking, this,
          &TransferListModel::handleTorrentStatusUpdated);
}
...
QVariant TransferListModel::headerData(const int section,
...
QString TransferListModel::displayValue(const BitTorrent::Torrent *torrent,
  ...
  if (m_hideZeroValuesMode == HideZeroValuesMode::Always)
    ...
    hideValues =
        (torrent->state() == BitTorrent::TorrentState::PausedDownloading);

  const auto availabilityString = [hideValues](const qreal value) -> QString {
    if (hideValues && (value == 0))
      return {};
    return (value >= 0) ? Utils::String::fromDouble(value, 3) : tr("N/A");
  };
  ...
  const auto limitString = [hideValues](const qint64 value) -> QString {
    ...
    return (value > 0) ? Utils::Misc::friendlyUnit(value, true) : C_INFINITY;
  };

  const auto amountString = [hideValues](const qint64 value,
  ...
  const auto ratioString = [hideValues](const qreal value) -> QString {
    ...
    return ((static_cast<int>(value) == -1) ||
            (value > BitTorrent::Torrent::MAX_RATIO))
               ? C_INFINITY
               : Utils::String::fromDouble(value, 2);
  };
  ...
  const auto lastActivityString = [hideValues](qint64 value) -> QString {
    ...
    return (value >= 0) ? tr("%1 ago", "e.g.: 1h 20m ago")
                              .arg(Utils::Misc::userFriendlyDuration(value))
                        : Utils::Misc::userFriendlyDuration(value);
  };

  const auto timeElapsedString =
      [hideValues](const qint64 elapsedTime,
  ...
  case TR_ADD_DATE:
    return QLocale().toString(torrent->addedTime().toLocalTime(),
                              QLocale::ShortFormat);
  case TR_SEED_DATE:
    return QLocale().toString(torrent->completedTime().toLocalTime(),
                              QLocale::ShortFormat);
  case TR_TRACKER:
  ...
  case TR_SEEN_COMPLETE_DATE:
    return QLocale().toString(torrent->lastSeenComplete().toLocalTime(),
                              QLocale::ShortFormat);
  case TR_LAST_ACTIVITY:
...
bool TransferListModel::setData(const QModelIndex &index, const QVariant &value,
                                int role) {
...
void TransferListModel::addTorrents(
...
BitTorrent::Torrent *
TransferListModel::torrentHandle(const QModelIndex &index) const {
  if (!index.isValid())
    return nullptr;
...
void TransferListModel::handleTorrentAboutToBeRemoved(
...
void TransferListModel::handleTorrentStatusUpdated(
    BitTorrent::Torrent *const torrent) {
  const int row = m_torrentMap.value(torrent, -1);
  Q_ASSERT(row >= 0);
...
QIcon TransferListModel::getIconByState(
    const BitTorrent::TorrentState state) const {
  switch (state) {
