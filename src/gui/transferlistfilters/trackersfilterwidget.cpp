...
namespace {
enum TRACKER_FILTER_ROW { ALL_ROW, TRACKERLESS_ROW, ERROR_ROW, WARNING_ROW };
...
TrackersFilterWidget::TrackersFilterWidget(QWidget *parent,
  ...
  allTrackers->setData(Qt::DisplayRole,
                       tr("All (0)", "this is for the tracker filter"));
  allTrackers->setData(
      Qt::DecorationRole,
  ...
  noTracker->setData(Qt::DecorationRole,
                     UIThemeManager::instance()->getIcon(u"trackerless"_s,
                                                         u"network-server"_s));
  auto *errorTracker = new QListWidgetItem(this);
  errorTracker->setData(Qt::DisplayRole, tr("Error (0)"));
  errorTracker->setData(Qt::DecorationRole,
                        UIThemeManager::instance()->getIcon(u"tracker-error"_s,
                                                            u"dialog-error"_s));
  auto *warningTracker = new QListWidgetItem(this);
  warningTracker->setData(Qt::DisplayRole, tr("Warning (0)"));
  warningTracker->setData(Qt::DecorationRole,
                          UIThemeManager::instance()->getIcon(
                              u"tracker-warning"_s, u"dialog-warning"_s));
...
void TrackersFilterWidget::addTrackers(
...
void TrackersFilterWidget::removeTrackers(const BitTorrent::Torrent *torrent,
...
void TrackersFilterWidget::refreshTrackers(const BitTorrent::Torrent *torrent) {
  ...
  Algorithm::removeIf(m_trackers, [this, &torrentID](const QString &host,
    ...
    trackerItem->setText(
        u"%1 (%2)"_s.arg((host.isEmpty() ? tr("Trackerless") : host),
                         QString::number(torrentIDs.size())));
    return false;
  });
...
void TrackersFilterWidget::changeTrackerless(const BitTorrent::Torrent *torrent,
                                             const bool trackerless) {
  if (trackerless)
...
void TrackersFilterWidget::addItems(
    const QString &trackerURL, const QVector<BitTorrent::TorrentID> &torrents) {
  ...
  if (exists) {
    ...
    trackerItem->setData(Qt::DecorationRole,
                         UIThemeManager::instance()->getIcon(
                             u"trackers"_s, u"network-server"_s));
    ...
    downloadFavicon(host, u"%1://%2/favicon.ico"_s.arg(
                              (scheme.startsWith(u"http") ? scheme : u"http"_s),
                              getFaviconHost(host)));
  }
  ...
  trackerItem->setText(
      u"%1 (%2)"_s.arg(((host == NULL_HOST) ? tr("Trackerless") : host),
                       QString::number(torrentIDs.size())));
  if (exists) {
...
void TrackersFilterWidget::removeItem(const QString &trackerURL,
  ...
  if (!host.isEmpty()) {
    ...
    if (trackerItem)
      trackerItem->setText(
          u"%1 (%2)"_s.arg(host, QString::number(torrentIDs.size())));
  } else {
  ...
  if (m_downloadTrackerFavicon) {
    for (auto i = m_trackers.cbegin(); i != m_trackers.cend(); ++i) {
      ...
      if (!tracker.isEmpty()) {
        ...
        downloadFavicon(tracker,
                        u"%1://%2/favicon.ico"_s.arg(
                            (scheme.startsWith(u"http") ? scheme : u"http"_s),
                            getFaviconHost(tracker)));
      }
    }
  }
}
...
void TrackersFilterWidget::downloadFavicon(const QString &trackerHost,
...
void TrackersFilterWidget::handleFavicoDownloadFinished(
  ...
  if (result.status != Net::DownloadStatus::Success) {
    ...
    const bool invalid =
        (sizes.isEmpty() || icon.pixmap(sizes.first()).isNull());
    if (invalid) {
  ...
  if (failed) {
    if (result.url.endsWith(u".ico", Qt::CaseInsensitive)) {
      const QString faviconURL =
          result.url.left(result.url.size() - 4) + u".png";
...
void TrackersFilterWidget::handleTorrentsLoaded(
  ...
  for (auto it = torrentsPerTracker.cbegin(); it != torrentsPerTracker.cend();
       ++it) {
  ...
  item(ALL_ROW)->setText(
      tr("All (%1)", "this is for the tracker filter").arg(m_totalTorrents));
}

void TrackersFilterWidget::torrentAboutToBeDeleted(
...
QSet<BitTorrent::TorrentID>
TrackersFilterWidget::getTorrentIDs(const int row) const {
  switch (row) {
