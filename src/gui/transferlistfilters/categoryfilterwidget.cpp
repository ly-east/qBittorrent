...
namespace {
QString getCategoryFilter(const CategoryFilterProxyModel *const model,
...
CategoryFilterWidget::CategoryFilterWidget(QWidget *parent)
  ...
  connect(this, &QTreeView::collapsed, this,
          &CategoryFilterWidget::callUpdateGeometry);
  connect(this, &QTreeView::expanded, this,
          &CategoryFilterWidget::callUpdateGeometry);
  connect(this, &QWidget::customContextMenuRequested, this,
          &CategoryFilterWidget::showMenu);
  connect(selectionModel(), &QItemSelectionModel::currentRowChanged, this,
          &CategoryFilterWidget::onCurrentRowChanged);
...
QString CategoryFilterWidget::currentCategory() const {
  ...
  return getCategoryFilter(static_cast<CategoryFilterProxyModel *>(model()),
                           current);
}

void CategoryFilterWidget::onCurrentRowChanged(const QModelIndex &current,
                                               const QModelIndex &previous) {
  Q_UNUSED(previous);

  emit categoryChanged(getCategoryFilter(
      static_cast<CategoryFilterProxyModel *>(model()), current));
}
...
void CategoryFilterWidget::showMenu() {
  ...
  menu->addAction(UIThemeManager::instance()->getIcon(u"list-add"_s),
                  ...
                  &CategoryFilterWidget::addCategory);

  const auto selectedRows = selectionModel()->selectedRows();
  if (!selectedRows.empty() &&
      !CategoryFilterModel::isSpecialItem(selectedRows.first())) {
    if (BitTorrent::Session::instance()->isSubcategoriesEnabled()) {
      menu->addAction(UIThemeManager::instance()->getIcon(u"list-add"_s),
                      tr("Add subcategory..."), this,
                      &CategoryFilterWidget::addSubcategory);
    }

    menu->addAction(UIThemeManager::instance()->getIcon(u"edit-rename"_s,
                                                        u"document-edit"_s),
                    tr("Edit category..."), this,
                    &CategoryFilterWidget::editCategory);
    menu->addAction(
        UIThemeManager::instance()->getIcon(u"edit-clear"_s, u"list-remove"_s),
        tr("Remove category"), this, &CategoryFilterWidget::removeCategory);
  }

  menu->addAction(
      UIThemeManager::instance()->getIcon(u"edit-clear"_s, u"list-remove"_s),
      tr("Remove unused categories"), this,
...
void CategoryFilterWidget::rowsInserted(const QModelIndex &parent, int start,
                                        int end) {
...
void CategoryFilterWidget::removeCategory() {
  ...
  if (!selectedRows.empty() &&
    ...
    BitTorrent::Session::instance()->removeCategory(
        static_cast<CategoryFilterProxyModel *>(model())->categoryName(
            selectedRows.first()));
    updateGeometry();
  }
}
...
void CategoryFilterWidget::removeUnusedCategories() {
  ...
  for (const QString &category : asConst(session->categories())) {
    if (model()->data(
            static_cast<CategoryFilterProxyModel *>(model())->index(category),
            Qt::UserRole) == 0)
