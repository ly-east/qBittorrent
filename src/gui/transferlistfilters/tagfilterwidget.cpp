...
namespace {
QString getTagFilter(const TagFilterProxyModel *const model,
...
TagFilterWidget::TagFilterWidget(QWidget *parent) : QTreeView(parent) {
  ...
  connect(this, &TagFilterWidget::collapsed, this,
          &TagFilterWidget::callUpdateGeometry);
  connect(this, &TagFilterWidget::expanded, this,
          &TagFilterWidget::callUpdateGeometry);
  connect(this, &TagFilterWidget::customContextMenuRequested, this,
          &TagFilterWidget::showMenu);
...
void TagFilterWidget::onCurrentRowChanged(const QModelIndex &current,
  ...
  emit tagChanged(
      getTagFilter(static_cast<TagFilterProxyModel *>(model()), current));
}
...
void TagFilterWidget::showMenu() {
  ...
  menu->addAction(UIThemeManager::instance()->getIcon(u"list-add"_s),
                  tr("Add tag..."), this, &TagFilterWidget::addTag);

  const auto selectedRows = selectionModel()->selectedRows();
  if (!selectedRows.empty() &&
      !TagFilterModel::isSpecialItem(selectedRows.first())) {
    menu->addAction(
        UIThemeManager::instance()->getIcon(u"edit-clear"_s, u"list-remove"_s),
        tr("Remove tag"), this, &TagFilterWidget::removeTag);
  }

  menu->addAction(
      UIThemeManager::instance()->getIcon(u"edit-clear"_s, u"list-remove"_s),
      tr("Remove unused tags"), this, &TagFilterWidget::removeUnusedTags);
  menu->addSeparator();
...
void TagFilterWidget::callUpdateGeometry() { updateGeometry(); }
...
void TagFilterWidget::rowsInserted(const QModelIndex &parent, int start,
                                   int end) {
...
QString TagFilterWidget::askTagName() {
  ...
  while (invalid) {
    invalid = false;
    tag = AutoExpandableDialog::getText(this, tr("New Tag"), tr("Tag:"),
...
void TagFilterWidget::addTag() {
  ...
  if (BitTorrent::Session::instance()->tags().contains(tag))
    QMessageBox::warning(this, tr("Tag exists"),
                         tr("Tag name already exists."));
  else
...
void TagFilterWidget::removeUnusedTags() {
  ...
  for (const QString &tag : asConst(session->tags()))
    if (model()->data(static_cast<TagFilterProxyModel *>(model())->index(tag),
                      Qt::UserRole) == 0)
