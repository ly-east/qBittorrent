...
public:
  ...
  CategoryModelItem(CategoryModelItem *parent, const QString &categoryName,
                    const int torrentsCount = 0)
  ...
  CategoryModelItem *parent() const { return m_parent; }
  ...
  int pos() const {
    ...
    if (const int posByName = m_parent->m_childUids.indexOf(m_name);
        posByName >= 0)
      return posByName;
  ...
  bool hasChild(const QString &name) const { return m_children.contains(name); }
...
CategoryFilterModel::CategoryFilterModel(QObject *parent)
  ...
  connect(session, &Session::categoryAdded, this,
          &CategoryFilterModel::categoryAdded);
  connect(session, &Session::categoryRemoved, this,
          &CategoryFilterModel::categoryRemoved);
  connect(session, &Session::torrentCategoryChanged, this,
          &CategoryFilterModel::torrentCategoryChanged);
  connect(session, &Session::subcategoriesSupportChanged, this,
          &CategoryFilterModel::subcategoriesSupportChanged);
  connect(session, &Session::torrentsLoaded, this,
          &CategoryFilterModel::torrentsLoaded);
  connect(session, &Session::torrentAboutToBeRemoved, this,
          &CategoryFilterModel::torrentAboutToBeRemoved);

  populate();
}

CategoryFilterModel::~CategoryFilterModel() { delete m_rootItem; }
...
int CategoryFilterModel::columnCount(const QModelIndex &) const { return 1; }
...
QVariant CategoryFilterModel::data(const QModelIndex &index, int role) const {
  ...
  const auto *item =
      static_cast<const CategoryModelItem *>(index.internalPointer());

  if ((index.column() == 0) && (role == Qt::DecorationRole)) {
    return UIThemeManager::instance()->getIcon(u"view-categories"_s,
                                               u"inode-directory"_s);
  }

  if ((index.column() == 0) && (role == Qt::DisplayRole)) {
    return u"%1 (%2)"_s.arg(item->name(),
                            QString::number(item->torrentsCount()));
  }
...
QVariant CategoryFilterModel::headerData(int section,
...
QModelIndex CategoryFilterModel::index(int row, int column,
  ...
  auto *parentItem =
      parent.isValid()
          ? static_cast<CategoryModelItem *>(parent.internalPointer())
          : m_rootItem;
...
void CategoryFilterModel::categoryAdded(const QString &categoryName) {
  ...
  new CategoryModelItem(parent, m_isSubcategoriesEnabled
...
void CategoryFilterModel::torrentsLoaded(
...
void CategoryFilterModel::torrentAboutToBeRemoved(
...
void CategoryFilterModel::torrentCategoryChanged(
    BitTorrent::Torrent *const torrent, const QString &oldCategory) {
...
void CategoryFilterModel::populate() {
  ...
  // All torrents
  m_rootItem->addChild(
      CategoryModelItem::UID_ALL,
      new CategoryModelItem(nullptr, tr("All"), torrents.count()));

  // Uncategorized torrents
  using Torrent = BitTorrent::Torrent;
  const int torrentsCount =
      std::count_if(torrents.begin(), torrents.end(), [](Torrent *torrent) {
        return torrent->category().isEmpty();
      });
  m_rootItem->addChild(
      CategoryModelItem::UID_UNCATEGORIZED,
  ...
  if (m_isSubcategoriesEnabled) {
    for (const QString &categoryName : asConst(session->categories())) {
      CategoryModelItem *parent = m_rootItem;
      for (const QString &subcat :
           asConst(BitTorrent::Session::expandCategory(categoryName))) {
        const QString subcatName = shortName(subcat);
        if (!parent->hasChild(subcatName)) {
          const int torrentsCount = std::count_if(
              torrents.cbegin(), torrents.cend(), [subcat](Torrent *torrent) {
                return torrent->category() == subcat;
              });
    ...
    for (const QString &categoryName : asConst(session->categories())) {
      const int torrentsCount = std::count_if(
          torrents.begin(), torrents.end(), [categoryName](Torrent *torrent) {
            return torrent->belongsToCategory(categoryName);
          });
...
CategoryModelItem *
CategoryFilterModel::findItem(const QString &fullName) const {
  if (fullName.isEmpty())
  ...
  for (const QString &subcat :
       asConst(BitTorrent::Session::expandCategory(fullName))) {
    const QString subcatName = shortName(subcat);
