...
void ProgressBarPainter::paint(QPainter *painter,
                               ...
                               const QString &text, const int progress) const {
  ...
  // Qt 6 requires QStyle::State_Horizontal to be set for correctly drawing
  ...
  styleOption.palette.setCurrentColorGroup(isEnabled ? QPalette::Active
                                                     : QPalette::Disabled);
  ...
  style->drawPrimitive(QStyle::PE_PanelItemViewItem, &option, painter,
                       option.widget);
  style->drawControl(QStyle::CE_ProgressBar, &styleOption, painter,
                     &m_dummyProgressBar);
  painter->restore();
}
