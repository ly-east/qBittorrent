...
void TorrentContentItemDelegate::setEditorData(QWidget *editor,
...
QWidget *
TorrentContentItemDelegate::createEditor(QWidget *parent,
  ...
  // add Mixed priority item to the new combobox only for those items with Mixed
  // priority
  const auto priority = static_cast<BitTorrent::DownloadPriority>(
      index.data(TorrentContentModel::UnderlyingDataRole).toInt());
  ...
  connect(editor, qOverload<int>(&QComboBox::currentIndexChanged), this,
          [this, editor]() {
            emit const_cast<TorrentContentItemDelegate *>(this)->commitData(
                editor);
          });
...
void TorrentContentItemDelegate::setModelData(QWidget *editor,
                                              QAbstractItemModel *model,
                                              const QModelIndex &index) const {
  const auto *combobox = static_cast<QComboBox *>(editor);

  BitTorrent::DownloadPriority prio =
      BitTorrent::DownloadPriority::Normal; // NORMAL
...
void TorrentContentItemDelegate::updateEditorGeometry(
    QWidget *editor, const QStyleOptionViewItem &option,
    const QModelIndex &) const {
  editor->setGeometry(option.rect);
}

void TorrentContentItemDelegate::paint(QPainter *painter,
                                       const QStyleOptionViewItem &option,
                                       const QModelIndex &index) const {
  switch (index.column()) {
  case TorrentContentModelItem::COL_PROGRESS: {
    const int progress = static_cast<int>(
        index.data(TorrentContentModel::UnderlyingDataRole).toReal());
    const int priority =
        index.sibling(index.row(), TorrentContentModelItem::COL_PRIO)
            .data(TorrentContentModel::UnderlyingDataRole)
            .toInt();
    const bool isEnabled =
        static_cast<BitTorrent::DownloadPriority>(priority) !=
        BitTorrent::DownloadPriority::Ignored;

    QStyleOptionViewItem customOption{option};
    customOption.state.setFlag(QStyle::State_Enabled, isEnabled);

    m_progressBarPainter.paint(painter, customOption, index.data().toString(),
                               progress);
  } break;
  default:
