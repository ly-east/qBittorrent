...
bool isDownloadable(const QString &str) {
  return (Net::DownloadManager::hasSupportedScheme(str) ||
          ...
          || ((str.size() == 64) &&
              !str.contains(QRegularExpression(
                  u"[^0-9A-Fa-f]"_s))) // v2 hex-encoded SHA-256 info-hash
#endif
          || ((str.size() == 40) &&
              !str.contains(QRegularExpression(
                  u"[^0-9A-Fa-f]"_s))) // v1 hex-encoded SHA-1 info-hash
          || ((str.size() == 32) &&
              !str.contains(QRegularExpression(
                  u"[^2-7A-Za-z]"_s)))); // v1 Base32 encoded SHA-1 info-hash
}
} // namespace

DownloadFromURLDialog::DownloadFromURLDialog(QWidget *parent)
    : QDialog(parent), m_ui(new Ui::DownloadFromURLDialog),
      m_storeDialogSize(SETTINGS_KEY(u"Size"_s)) {
  m_ui->setupUi(this);

  m_ui->buttonBox->button(QDialogButtonBox::Ok)->setText(tr("Download"));
  connect(m_ui->buttonBox, &QDialogButtonBox::accepted, this,
          &DownloadFromURLDialog::onSubmit);
  ...
  const QList<QStringView> clipboardList =
      QStringView(clipboardText).split(u'\n');
...
void DownloadFromURLDialog::onSubmit() {
  ...
  if (urlList.isEmpty()) {
    QMessageBox::warning(this, tr("No URL entered"),
                         tr("Please type at least one URL."));
    return;
  }
