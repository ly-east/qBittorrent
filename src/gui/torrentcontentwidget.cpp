...
TorrentContentWidget::TorrentContentWidget(QWidget *parent)
  ...
  connect(m_model, &TorrentContentModel::renameFailed, this,
          [this](const QString &errorMessage) {
            RaisedMessageBox::warning(this, tr("Rename error"), errorMessage,
                                      QMessageBox::Ok);
          });
  ...
  connect(this, &QAbstractItemView::doubleClicked, this,
          &TorrentContentWidget::onItemDoubleClicked);
  connect(this, &QWidget::customContextMenuRequested, this,
          &TorrentContentWidget::displayContextMenu);
  connect(header(), &QWidget::customContextMenuRequested, this,
          &TorrentContentWidget::displayColumnHeaderMenu);
  ...
  const auto *renameFileHotkey =
      new QShortcut(Qt::Key_F2, this, nullptr, nullptr, Qt::WidgetShortcut);
  connect(renameFileHotkey, &QShortcut::activated, this,
          &TorrentContentWidget::renameSelectedFile);

  connect(model(), &QAbstractItemModel::modelReset, this,
          &TorrentContentWidget::expandRecursively);
}

void TorrentContentWidget::setContentHandler(
...
void TorrentContentWidget::setOpenByEnterKey(const bool value) {
  ...
  if (value) {
    m_openFileHotkeyReturn = new QShortcut(Qt::Key_Return, this, nullptr,
                                           nullptr, Qt::WidgetShortcut);
    connect(m_openFileHotkeyReturn, &QShortcut::activated, this,
            &TorrentContentWidget::openSelectedFile);
    m_openFileHotkeyEnter = new QShortcut(Qt::Key_Enter, this, nullptr, nullptr,
                                          Qt::WidgetShortcut);
    connect(m_openFileHotkeyEnter, &QShortcut::activated, this,
            &TorrentContentWidget::openSelectedFile);
  } else {
...
TorrentContentWidget::ColumnsVisibilityMode
TorrentContentWidget::columnsVisibilityMode() const {
  return m_columnsVisibilityMode;
}

void TorrentContentWidget::setColumnsVisibilityMode(
    ColumnsVisibilityMode mode) {
...
void TorrentContentWidget::checkAll() {
  for (int i = 0; i < model()->rowCount(); ++i)
    model()->setData(model()->index(i, TorrentContentModelItem::COL_NAME),
...
void TorrentContentWidget::renameSelectedFile() {
  ...
  // Ask for new name
  const bool isFile = (m_filterModel->itemType(modelIndex) ==
                       TorrentContentModelItem::FileType);
  bool ok = false;
  QString newName =
      AutoExpandableDialog::getText(this, tr("Renaming"), tr("New name:"),
                                    QLineEdit::Normal,
                                    modelIndex.data().toString(), &ok, isFile)
          .trimmed();
...
void TorrentContentWidget::applyPrioritiesByOrder() {
  ...
  const auto priorityGroupSize =
      std::max<qsizetype>((selectedRows.length() / priorityGroups), 1);
...
void TorrentContentWidget::setModel(
...
void TorrentContentWidget::displayColumnHeaderMenu() {
  ...
  if (m_columnsVisibilityMode == ColumnsVisibilityMode::Editable) {
    ...
    for (int i = 0; i < TorrentContentModelItem::NB_COL; ++i) {
      ...
      QAction *action =
          menu->addAction(columnName, this, [this, i](bool checked) {
...
void TorrentContentWidget::displayContextMenu() {
  ...
  if (selectedRows.size() == 1) {
    ...
    if (!contentHandler()->actualStorageLocation().isEmpty()) {
      menu->addAction(
          UIThemeManager::instance()->getIcon(u"folder-documents"_s),
          tr("Open"), this, [this, index]() { openItem(index); });
      menu->addAction(UIThemeManager::instance()->getIcon(u"directory"_s),
    ...
    subMenu->addAction(tr("By shown file order"), this,
                       &TorrentContentWidget::applyPrioritiesByOrder);
  } else {
    ...
    menu->addSeparator();
    menu->addAction(tr("Priority by shown file order"), this,
                    &TorrentContentWidget::applyPrioritiesByOrder);
  }
...
Path TorrentContentWidget::getFullPath(const QModelIndex &index) const {
  ...
  if (const int fileIdx = getFileIndex(index); fileIdx >= 0) {
    const Path fullPath = contentHandler->actualStorageLocation() /
                          contentHandler->actualFilePath(fileIdx);
    return fullPath;
  }

  // folder type
  const Path fullPath =
      contentHandler->actualStorageLocation() / getItemPath(index);
  return fullPath;
}
