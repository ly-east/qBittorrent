...
namespace {
Path userConfigPath() {
  return specialFolderLocation(SpecialFolder::Config) /
         Path(u"themes/default"_s);
}

Path defaultIconPath(const QString &iconID,
                     [[maybe_unused]] const ColorMode colorMode) {
...
public:
  explicit ColorWidget(const QColor &currentColor, const QColor &defaultColor,
...
private:
  ...
  void resetColor() { setCurrentColor(m_defaultColor); }

  void applyColor(const QColor &color) {
    setStyleSheet(
        u"#colorWidget { background-color: %1; }"_s.arg(color.name()));
  }
  ...
  void showColorDialog() {
    ...
    connect(dialog, &QDialog::accepted, this,
            [this, dialog] { setCurrentColor(dialog->currentColor()); });
...
public:
  explicit IconWidget(const Path &currentPath, const Path &defaultPath,
                      QWidget *parent = nullptr)
...
private:
  ...
  void resetIcon() { setCurrentPath(m_defaultPath); }
  ...
  void showFileDialog() {
    auto *dialog =
        new QFileDialog(this, tr("Select icon"), QDir::homePath(),
                        (tr("Supported image files") + u" (*.svg *.png)"));
...
UIThemeDialog::UIThemeDialog(QWidget *parent)
    : QDialog(parent), m_ui{new Ui::UIThemeDialog},
...
void UIThemeDialog::accept() {
  ...
  if (hasError) {
    QMessageBox::critical(this, tr("UI Theme Configuration."),
                          tr("The UI Theme changes could not be fully applied. "
                             "The details can be found in the Log."));
  }
}

void UIThemeDialog::loadColors() {
  const QHash<QString, UIThemeColor> defaultColors = defaultUIThemeColors();
  const QList<QString> colorIDs = std::invoke(
      [](auto &&list) {
        list.sort();
        return list;
      },
  ...
  for (const QString &id : colorIDs) {
    ...
    auto *lightColorWidget =
        new ColorWidget(m_defaultThemeSource.getColor(id, ColorMode::Light),
                        defaultColor.light, this);
    m_lightColorWidgets.insert(id, lightColorWidget);
    m_ui->colorsLayout->addWidget(lightColorWidget, row, 2);

    auto *darkColorWidget =
        new ColorWidget(m_defaultThemeSource.getColor(id, ColorMode::Dark),
                        defaultColor.dark, this);
...
void UIThemeDialog::loadIcons() {
  ...
  const QList<QString> iconIDs = std::invoke(
      [](auto &&list) {
        list.sort();
        return list;
      },
  ...
  for (const QString &id : iconIDs) {
    ...
    auto *lightIconWidget =
        new IconWidget(m_defaultThemeSource.getIconPath(id, ColorMode::Light),
    ...
    auto *darkIconWidget =
        new IconWidget(m_defaultThemeSource.getIconPath(id, ColorMode::Dark),
...
bool UIThemeDialog::storeColors() {
  ...
  const QHash<QString, UIThemeColor> defaultColors = defaultUIThemeColors();
  const auto addColorOverrides = [this, &defaultColors,
                                  &userConfig](const ColorMode colorMode) {
    const QHash<QString, ColorWidget *> &colorWidgets =
        (colorMode == ColorMode::Light) ? m_lightColorWidgets
                                        : m_darkColorWidgets;
    ...
    for (auto it = colorWidgets.cbegin(); it != colorWidgets.cend(); ++it) {
      ...
      const QColor &defaultColor = (colorMode == ColorMode::Light)
                                       ? defaultColors.value(colorID).light
                                       : defaultColors.value(colorID).dark;
    ...
    if (!colors.isEmpty())
      userConfig.insert(((colorMode == ColorMode::Light) ? KEY_COLORS_LIGHT
                                                         : KEY_COLORS_DARK),
                        colors);
  };
  ...
  const nonstd::expected<void, QString> result = Utils::IO::saveToFile(
      (userConfigPath() / Path(CONFIG_FILE_NAME)), configData);
  if (!result) {
    const QString error = tr("Couldn't save UI Theme configuration. Reason: %1")
                              .arg(result.error());
...
bool UIThemeDialog::storeIcons() {
  ...
  const auto updateIcons = [this, &hasError](const ColorMode colorMode) {
    const QHash<QString, IconWidget *> &iconWidgets =
        (colorMode == ColorMode::Light) ? m_lightIconWidgets
                                        : m_darkIconWidgets;
    const Path subdirPath = (colorMode == ColorMode::Light)
                                ? Path(u"icons/light"_s)
                                : Path(u"icons/dark"_s);
    ...
    for (auto it = iconWidgets.cbegin(); it != iconWidgets.cend(); ++it) {
      ...
      if (const Path oldIconPath = userIconPathBase + u".svg";
        ...
        const QString error = tr("Couldn't remove icon file. File: %1.")
                                  .arg(oldIconPath.toString());
        LogMsg(error, Log::WARNING);
        hasError = true;
        continue;
      }
