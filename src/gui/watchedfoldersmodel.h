...
public:
  explicit WatchedFoldersModel(TorrentFilesWatcher *fsWatcher,
  ...
  void addFolder(const Path &path,
  ...
  void
  setFolderOptions(int row,
                   const TorrentFilesWatcher::WatchedFolderOptions &options);

  void apply();

private:
  void onFolderSet(const Path &path,
                   const TorrentFilesWatcher::WatchedFolderOptions &options);
  void onFolderRemoved(const Path &path);

  TorrentFilesWatcher *m_fsWatcher = nullptr;
  PathList m_watchedFolders;
  QHash<Path, TorrentFilesWatcher::WatchedFolderOptions>
