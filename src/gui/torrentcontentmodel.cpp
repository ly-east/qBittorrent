...
public:
  UnifiedFileIconProvider()
      : m_textPlainIcon{UIThemeManager::instance()->getIcon(u"help-about"_s,
                                                            u"text-plain"_s)} {}

  using QFileIconProvider::icon;

  QIcon icon(const QFileInfo &) const override { return m_textPlainIcon; }
...
class WinShellFileIconProvider final : public CachingFileIconProvider {
  QPixmap pixmapForExtension(const QString &ext) const override {
    ...
    SHFILEINFOW sfi{};
    const HRESULT hr =
        ::SHGetFileInfoW(extWStr.c_str(), FILE_ATTRIBUTE_NORMAL, &sfi,
 ...
 * QFileIconProvider::icon() returns empty icons as the result. Here we ask it
 * for two icons for probably absent files and when both icons are null, we
 * assume that the current QPA plugin does not implement
 * QPlatformTheme::fileIcon().
 */
bool doesQFileIconProviderWork() {
  const Path PSEUDO_UNIQUE_FILE_NAME =
      Utils::Fs::tempPath() /
      Path(
          u"qBittorrent-test-QFileIconProvider-845eb448-7ad5-4cdb-b764-b3f322a266a9"_s);
  QFileIconProvider provider;
  const QIcon testIcon1 =
      provider.icon(QFileInfo((PSEUDO_UNIQUE_FILE_NAME + u".pdf").data()));
  const QIcon testIcon2 =
      provider.icon(QFileInfo((PSEUDO_UNIQUE_FILE_NAME + u".png").data()));
...
class MimeFileIconProvider final : public UnifiedFileIconProvider {
  ...
  QIcon icon(const QFileInfo &info) const override {
    const QMimeType mimeType =
        QMimeDatabase().mimeTypeForFile(info, QMimeDatabase::MatchExtension);
...
TorrentContentModel::TorrentContentModel(QObject *parent)
    : QAbstractItemModel(parent),
      m_rootItem(new TorrentContentModelFolder(QVector<QString>(
          {tr("Name"), tr("Total Size"), tr("Progress"),
...
#else
      ,
      m_fileIconProvider{doesQFileIconProviderWork() ? new QFileIconProvider
                                                     : new MimeFileIconProvider}
#endif
{
  m_fileIconProvider->setOptions(
...
void TorrentContentModel::updateFilesPriorities() {
  ...
  const QVector<BitTorrent::DownloadPriority> fprio =
      m_contentHandler->filePriorities();
  ...
  for (int i = 0; i < fprio.size(); ++i)
    m_filesIndex[i]->setPriority(
        static_cast<BitTorrent::DownloadPriority>(fprio[i]));
}
...
void TorrentContentModel::updateFilesAvailability() {
  ...
  m_contentHandler->fetchAvailableFileFractions(
      [this, handler = HandlerPtr(m_contentHandler)](
          const QVector<qreal> &availableFileFractions) {
...
bool TorrentContentModel::setItemPriority(
    const QModelIndex &index, BitTorrent::DownloadPriority priority) {
...
bool TorrentContentModel::setData(const QModelIndex &index,
  ...
  if ((index.column() == TorrentContentModelItem::COL_NAME) &&
    ...
    const BitTorrent::DownloadPriority newPrio =
  ...
  if (role == Qt::EditRole) {
    auto *item =
        static_cast<TorrentContentModelItem *>(index.internalPointer());
    ...
    case TorrentContentModelItem::COL_PRIO: {
      const auto newPrio =
          static_cast<BitTorrent::DownloadPriority>(value.toInt());
      return setItemPriority(index, newPrio);
    } break;
...
TorrentContentModelItem::ItemType
TorrentContentModel::itemType(const QModelIndex &index) const {
  return static_cast<const TorrentContentModelItem *>(index.internalPointer())
      ->itemType();
}
...
QVariant TorrentContentModel::data(const QModelIndex &index,
  ...
  case Qt::CheckStateRole:
    ...
    if (item->priority() == BitTorrent::DownloadPriority::Mixed) {
      ...
      const bool hasIgnored =
          std::any_of(childItems.cbegin(), childItems.cend(),
...
QModelIndex TorrentContentModel::index(const int row, const int column,
...
int TorrentContentModel::rowCount(const QModelIndex &parent) const {
  const TorrentContentModelFolder *parentItem =
      parent.isValid() ? dynamic_cast<TorrentContentModelFolder *>(
...
void TorrentContentModel::populate() {
  ...
  QHash<TorrentContentModelFolder *,
        QHash<QString, TorrentContentModelFolder *>>
      folderMap;
  ...
  for (int i = 0; i < filesCount; ++i) {
    ...
    QList<QStringView> pathFolders =
        QStringView(path).split(u'/', Qt::SkipEmptyParts);
    ...
    if (!std::equal(lastParentPath.begin(), lastParentPath.end(),
      ...
      for (const QStringView pathPart : asConst(pathFolders)) {
        ...
        TorrentContentModelFolder *&newParent =
            folderMap[lastParent][folderName];
        if (!newParent) {
    ...
    auto *fileItem = new TorrentContentModelFile(
        fileName, m_contentHandler->fileSize(i), lastParent, i);
...
void TorrentContentModel::setContentHandler(
    BitTorrent::TorrentContentHandler *contentHandler) {
  beginResetModel();
  [[maybe_unused]] const auto modelResetGuard =
...
void TorrentContentModel::notifySubtreeUpdated(
    const QModelIndex &index, const QVector<ColumnInterval> &columns) {
  // For best performance, `columns` entries should be arranged from left to
  // right
  ...
  for (const ColumnInterval &column : columns)
    emit dataChanged(index.siblingAtColumn(column.first()),
                     index.siblingAtColumn(column.last()));

  // propagate up the model
  QModelIndex parentIndex = parent(index);
  while (parentIndex.isValid()) {
    for (const ColumnInterval &column : columns)
      emit dataChanged(parentIndex.siblingAtColumn(column.first()),
                       parentIndex.siblingAtColumn(column.last()));
  ...
  while (!parentIndexes.isEmpty()) {
    ...
    for (const ColumnInterval &column : columns) {
      ...
      const QModelIndex childBottomRight =
          child.sibling((childCount - 1), column.last());
