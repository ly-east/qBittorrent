...
const QString KEY_SAVEPATHHISTORYLENGTH =
    SETTINGS_KEY(u"SavePathHistoryLength"_s);

// just a shortcut
inline SettingsStorage *settings() { return SettingsStorage::instance(); }

// savePath is a folder, not an absolute file path
int indexOfPath(const FileSystemPathComboEdit *fsPathEdit,
                const Path &savePath) {
...
qint64 queryFreeDiskSpace(const Path &path) {
  ...
  // for non-existent directories (which will be created on demand)
  // `Utils::Fs::freeDiskSpaceOnPath` will return invalid value so instead query
  // its parent/ancestor paths
...
void updatePathHistory(const QString &settingsKey, const Path &path,
                       const int maxLength) {
...
void adjustDialogGeometry(QWidget *dialog, const QWidget *parentWindow) {
  ...
  // However, if it goes beyond the current screen, then move it so that it fits
  // there (or, if the dialog is larger than the current screen, at least make
  // sure that the upper/left coordinates of the dialog are inside it).
...
public:
  ...
  bool hasMetadata() const override { return m_torrentInfo.isValid(); }

  int filesCount() const override { return m_torrentInfo.filesCount(); }
  ...
  Path filePath(const int index) const override {
    Q_ASSERT((index >= 0) && (index < filesCount()));
    return (m_filePaths.isEmpty() ? m_torrentInfo.filePath(index)
                                  : m_filePaths.at(index));
  }
  ...
  void
  applyContentLayout(const BitTorrent::TorrentContentLayout contentLayout) {
    Q_ASSERT(hasMetadata());
    Q_ASSERT(!m_filePaths.isEmpty());

    const auto originalContentLayout =
        (m_originalRootFolder.isEmpty()
             ? BitTorrent::TorrentContentLayout::NoSubfolder
             : BitTorrent::TorrentContentLayout::Subfolder);
    const auto newContentLayout =
    ...
    if (newContentLayout != m_currentContentLayout) {
      if (newContentLayout == BitTorrent::TorrentContentLayout::NoSubfolder) {
        ...
        const auto rootFolder = ((originalContentLayout ==
                                  BitTorrent::TorrentContentLayout::Subfolder)
  ...
  QVector<BitTorrent::DownloadPriority> filePriorities() const override {
    return m_filePriorities.isEmpty()
               ? QVector<BitTorrent::DownloadPriority>(
  ...
  void fetchAvailableFileFractions(
      std::function<void(QVector<qreal>)> resultHandler) const override {
    resultHandler(availableFileFractions());
  }

  void prioritizeFiles(
      const QVector<BitTorrent::DownloadPriority> &priorities) override {
...
AddNewTorrentDialog::AddNewTorrentDialog(
    ...
    : QDialog(parent), m_ui(new Ui::AddNewTorrentDialog),
      m_filterLine(new LineEdit(this)), m_torrentParams(inParams),
...
#endif
{
  ...
  connect(m_ui->buttonSave, &QPushButton::clicked, this,
          &AddNewTorrentDialog::saveTorrentFile);
  ...
  m_ui->addToQueueTopCheckBox->setChecked(
      m_torrentParams.addToQueueTop.value_or(
          session->isAddTorrentToQueueTop()));
  ...
  m_ui->stopConditionComboBox->setToolTip(
      ...
      u"</em></p></body></html>");
  m_ui->stopConditionComboBox->addItem(
      tr("None"),
      QVariant::fromValue(BitTorrent::Torrent::StopCondition::None));
  if (!hasMetadata())
    m_ui->stopConditionComboBox->addItem(
        tr("Metadata received"),
        QVariant::fromValue(
            BitTorrent::Torrent::StopCondition::MetadataReceived));
  m_ui->stopConditionComboBox->addItem(
      tr("Files checked"),
      QVariant::fromValue(BitTorrent::Torrent::StopCondition::FilesChecked));
  const auto stopCondition =
      m_torrentParams.stopCondition.value_or(session->torrentStopCondition());
  if (hasMetadata() &&
    ...
    m_ui->startTorrentCheckBox->setChecked(
        !m_torrentParams.addPaused.value_or(session->isAddTorrentPaused()));
    m_ui->stopConditionComboBox->setCurrentIndex(
        m_ui->stopConditionComboBox->findData(
            QVariant::fromValue(stopCondition)));
  }
  m_ui->stopConditionLabel->setEnabled(m_ui->startTorrentCheckBox->isChecked());
  m_ui->stopConditionComboBox->setEnabled(
      m_ui->startTorrentCheckBox->isChecked());
  connect(m_ui->startTorrentCheckBox, &QCheckBox::toggled, this,
          [this](const bool checked) {
            m_ui->stopConditionLabel->setEnabled(checked);
            m_ui->stopConditionComboBox->setEnabled(checked);
          });

  m_ui->comboTTM->blockSignals(true); // the TreeView size isn't correct if the
                                      // slot does its job at this point
  m_ui->comboTTM->setCurrentIndex(session->isAutoTMMDisabledByDefault() ? 0
                                                                        : 1);
  m_ui->comboTTM->blockSignals(false);
  connect(m_ui->comboTTM, &QComboBox::currentIndexChanged, this,
          &AddNewTorrentDialog::TMMChanged);

  connect(m_ui->savePath, &FileSystemPathEdit::selectedPathChanged, this,
          &AddNewTorrentDialog::onSavePathChanged);
  connect(m_ui->downloadPath, &FileSystemPathEdit::selectedPathChanged, this,
          &AddNewTorrentDialog::onDownloadPathChanged);
  connect(m_ui->groupBoxDownloadPath, &QGroupBox::toggled, this,
          &AddNewTorrentDialog::onUseDownloadPathChanged);

  m_ui->checkBoxRememberLastSavePath->setChecked(m_storeRememberLastSavePath);

  m_ui->contentLayoutComboBox->setCurrentIndex(static_cast<int>(
      m_torrentParams.contentLayout.value_or(session->torrentContentLayout())));
  connect(m_ui->contentLayoutComboBox, &QComboBox::currentIndexChanged, this,
          &AddNewTorrentDialog::contentLayoutChanged);
  ...
  m_ui->doNotDeleteTorrentCheckBox->setVisible(
  ...
  connect(m_ui->categoryComboBox, &QComboBox::currentIndexChanged, this,
          &AddNewTorrentDialog::categoryChanged);
  ...
  connect(m_filterLine, &LineEdit::textChanged, m_ui->contentTreeView,
          &TorrentContentWidget::setFilterPattern);
  m_ui->contentFilterLayout->insertWidget(3, m_filterLine);

  loadState();

  connect(m_ui->doNotDeleteTorrentCheckBox, &QCheckBox::clicked, this,
          &AddNewTorrentDialog::doNotDeleteTorrentClicked);

  connect(m_ui->buttonSelectAll, &QPushButton::clicked, m_ui->contentTreeView,
          &TorrentContentWidget::checkAll);
  connect(m_ui->buttonSelectNone, &QPushButton::clicked, m_ui->contentTreeView,
  ...
  m_ui->contentTreeView->setColumnsVisibilityMode(
      TorrentContentWidget::ColumnsVisibilityMode::Locked);
  m_ui->contentTreeView->setDoubleClickAction(
      TorrentContentWidget::DoubleClickAction::Rename);

  m_ui->labelCommentData->setText(
      tr("Not Available", "This comment is unavailable"));
...
int AddNewTorrentDialog::savePathHistoryLength() {
  ...
  const int value =
      settings()->loadValue(KEY_SAVEPATHHISTORYLENGTH, defaultHistoryLength);
...
void AddNewTorrentDialog::setSavePathHistoryLength(const int value) {
  const int clampedValue =
      qBound(minPathHistoryLength, value, maxPathHistoryLength);
  ...
  settings()->storeValue(
      ...
      QStringList(settings()
                      ->loadValue<QStringList>(KEY_SAVEPATHHISTORY)
                      .mid(0, clampedValue)));
}
...
void AddNewTorrentDialog::show(const QString &source,
  ...
  // Qt::Window is required to avoid showing only two dialog on top (see
  // #12852). Also improves the general convenience of adding multiple torrents.
  if (!attached) {
  ...
  if (Net::DownloadManager::hasSupportedScheme(source)) {
    ...
    Net::DownloadManager::instance()->download(
        ...
        pref->useProxyForGeneralPurposes(), dlg,
        &AddNewTorrentDialog::handleDownloadFinished);
    return;
  }

  const BitTorrent::MagnetUri magnetUri{source};
  const bool isLoaded = magnetUri.isValid() ? dlg->loadMagnet(magnetUri)
...
bool AddNewTorrentDialog::loadTorrentFile(const QString &source) {
  const Path decodedPath{
      source.startsWith(u"file://", Qt::CaseInsensitive)
          ? QUrl::fromEncoded(source.toLocal8Bit()).toLocalFile()
          : source};

  const nonstd::expected<BitTorrent::TorrentInfo, QString> result =
      BitTorrent::TorrentInfo::loadFromFile(decodedPath);
  if (!result) {
    RaisedMessageBox::critical(
        this, tr("Invalid torrent"),
    ...
    if (torrent) {
      ...
      if (torrent->isPrivate() || m_torrentInfo.isPrivate()) {
        RaisedMessageBox::warning(
            this, tr("Torrent is already present"),
            tr("Torrent '%1' is already in the transfer list. Trackers cannot "
               "be merged because it is a private torrent.")
                .arg(torrent->name()),
            QMessageBox::Ok);
      } else {
        bool mergeTrackers = btSession->isMergeTrackersEnabled();
        if (Preferences::instance()->confirmMergeTrackers()) {
          const QMessageBox::StandardButton btn = RaisedMessageBox::question(
              this, tr("Torrent is already present"),
              tr("Torrent '%1' is already in the transfer list. Do you want to "
                 "merge trackers from new source?")
                  .arg(torrent->name()),
      ...
      RaisedMessageBox::information(
          this, tr("Torrent is already present"),
          tr("Torrent is already queued for processing."), QMessageBox::Ok);
    }

    return false;
  }

  m_ui->labelInfohash1Data->setText(
      m_torrentInfo.infoHash().v1().isValid()
          ? m_torrentInfo.infoHash().v1().toString()
          : tr("N/A"));
  m_ui->labelInfohash2Data->setText(
      m_torrentInfo.infoHash().v2().isValid()
          ? m_torrentInfo.infoHash().v2().toString()
          : tr("N/A"));
  setupTreeview();
...
bool AddNewTorrentDialog::loadMagnet(const BitTorrent::MagnetUri &magnetUri) {
  if (!magnetUri.isValid()) {
    RaisedMessageBox::critical(this, tr("Invalid magnet link"),
                               tr("This magnet link was not recognized"));
    return false;
  }
  ...
  if (btSession->isKnownTorrent(infoHash)) {
    ...
    if (torrent) {
      if (torrent->isPrivate()) {
        RaisedMessageBox::warning(
            this, tr("Torrent is already present"),
            tr("Torrent '%1' is already in the transfer list. Trackers haven't "
               "been merged because it is a private torrent.")
                .arg(torrent->name()),
            QMessageBox::Ok);
      } else {
        bool mergeTrackers = btSession->isMergeTrackersEnabled();
      ...
      RaisedMessageBox::information(
          this, tr("Torrent is already present"),
          tr("Magnet link is already queued for processing."), QMessageBox::Ok);
    }

    return false;
  }

  connect(btSession, &BitTorrent::Session::metadataDownloaded, this,
          &AddNewTorrentDialog::updateMetadata);
  ...
  m_ui->labelInfohash1Data->setText(magnetUri.infoHash().v1().isValid()
                                        ? magnetUri.infoHash().v1().toString()
                                        : tr("N/A"));
  m_ui->labelInfohash2Data->setText(magnetUri.infoHash().v2().isValid()
                                        ? magnetUri.infoHash().v2().toString()
                                        : tr("N/A"));
...
void AddNewTorrentDialog::updateDiskSpaceLabel() {
  ...
  if (hasMetadata()) {
    const QVector<BitTorrent::DownloadPriority> &priorities =
        m_contentAdaptor->filePriorities();
  ...
  const QString freeSpace = Utils::Misc::friendlyUnit(
      queryFreeDiskSpace(m_ui->savePath->selectedPath()));
  const QString sizeString =
      tr("%1 (Free space on disk: %2)")
          .arg(((torrentSize > 0)
                    ? Utils::Misc::friendlyUnit(torrentSize)
...
void AddNewTorrentDialog::categoryChanged(int index) {
  ...
  if (m_ui->comboTTM->currentIndex() == 1) {
    ...
    m_ui->groupBoxDownloadPath->setChecked(
...
void AddNewTorrentDialog::contentLayoutChanged() {
  ...
  const auto contentLayout = static_cast<BitTorrent::TorrentContentLayout>(
      m_ui->contentLayoutComboBox->currentIndex());
  m_contentAdaptor->applyContentLayout(contentLayout);
  m_ui->contentTreeView->setContentHandler(
      m_contentAdaptor.get()); // to cause reloading
}
...
void AddNewTorrentDialog::saveTorrentFile() {
  ...
  Path path{QFileDialog::getSaveFileName(
      ...
      QDir::home().absoluteFilePath(m_torrentInfo.name() +
  ...
  if (!result) {
    QMessageBox::critical(
        this, tr("I/O Error"),
        tr("Couldn't export torrent metadata file '%1'. Reason: %2.")
...
void AddNewTorrentDialog::populateSavePaths() {
  ...
  m_ui->savePath->clear();
  const auto savePathHistory =
      settings()->loadValue<QStringList>(KEY_SAVEPATHHISTORY);
  ...
  if (m_savePathIndex >= 0) {
    m_ui->savePath->setCurrentIndex(
        std::min(m_savePathIndex, (m_ui->savePath->count() - 1)));
  } else {
  ...
  const auto downloadPathHistory =
      settings()->loadValue<QStringList>(KEY_DOWNLOADPATHHISTORY);
  ...
  if (m_downloadPathIndex >= 0) {
    m_ui->downloadPath->setCurrentIndex(
        m_useDownloadPath
            ? std::min(m_downloadPathIndex, (m_ui->downloadPath->count() - 1))
            : -1);
    m_ui->groupBoxDownloadPath->setChecked(m_useDownloadPath);
  } else {
    const bool useDownloadPath = m_torrentParams.useDownloadPath.value_or(
        btSession->isDownloadPathEnabled());
...
void AddNewTorrentDialog::accept() {
  ...
  m_torrentParams.stopCondition =
      m_ui->stopConditionComboBox->currentData()
          .value<BitTorrent::Torrent::StopCondition>();
  m_torrentParams.contentLayout = static_cast<BitTorrent::TorrentContentLayout>(
      m_ui->contentLayoutComboBox->currentIndex());
  ...
  const bool useAutoTMM =
      (m_ui->comboTTM->currentIndex() ==
       1); // 1 is Automatic mode. Handle all non 1 values as manual mode.
  ...
  if (!useAutoTMM) {
    ...
    if (m_torrentParams.useDownloadPath) {
      ...
      updatePathHistory(KEY_DOWNLOADPATHHISTORY, downloadPath,
...
void AddNewTorrentDialog::reject() {
  if (!hasMetadata()) {
    ...
    BitTorrent::Session::instance()->cancelDownloadMetadata(
        m_magnetURI.infoHash().toTorrentID());
  }

  QDialog::reject();
}

void AddNewTorrentDialog::updateMetadata(
    const BitTorrent::TorrentInfo &metadata) {
  Q_ASSERT(metadata.isValid());

  if (metadata.infoHash() != m_magnetURI.infoHash())
    return;

  disconnect(BitTorrent::Session::instance(),
             &BitTorrent::Session::metadataDownloaded, this,
             &AddNewTorrentDialog::updateMetadata);
  ...
  if (m_torrentInfo.infoHash().v2().isValid()) {
    ...
    m_ui->buttonSave->setToolTip(
        tr("Cannot create v2 torrent until its data is fully downloaded."));
  }
}

void AddNewTorrentDialog::setMetadataProgressIndicator(
    bool visibleIndicator, const QString &labelText) {
...
void AddNewTorrentDialog::setupTreeview() {
  ...
  m_ui->labelCommentData->setText(
      Utils::Misc::parseHtmlLinks(m_torrentInfo.comment().toHtmlEscaped()));
  m_ui->labelDateData->setText(
      !m_torrentInfo.creationDate().isNull()
          ? QLocale().toString(m_torrentInfo.creationDate(),
                               QLocale::ShortFormat)
          : tr("Not available"));
  ...
  m_contentAdaptor = std::make_unique<TorrentContentAdaptor>(
      m_torrentInfo, m_torrentParams.filePaths, m_torrentParams.filePriorities,
  ...
  if (BitTorrent::Session::instance()->isExcludedFileNamesEnabled()) {
    ...
    QVector<BitTorrent::DownloadPriority> priorities =
        m_contentAdaptor->filePriorities();
    for (int i = 0; i < priorities.size(); ++i) {
      if (priorities[i] == BitTorrent::DownloadPriority::Ignored)
        continue;

      if (BitTorrent::Session::instance()->isFilenameExcluded(
              m_torrentInfo.filePath(i).filename()))
...
void AddNewTorrentDialog::handleDownloadFinished(
    const Net::DownloadResult &downloadResult) {
  switch (downloadResult.status) {
  case Net::DownloadStatus::Success: {
    const nonstd::expected<BitTorrent::TorrentInfo, QString> result =
        BitTorrent::TorrentInfo::load(downloadResult.data);
    if (!result) {
  ...
  default:
    RaisedMessageBox::critical(
        ...
        tr("Cannot download '%1': %2")
            .arg(downloadResult.url, downloadResult.errorString));
    deleteLater();
  }
}

void AddNewTorrentDialog::TMMChanged(int index) {
  if (index != 1) { // 0 is Manual mode and 1 is Automatic mode. Handle all non
                    // 1 values as manual mode.
    populateSavePaths();
    ...
    m_ui->savePath->clear();
    const Path savePath =
        session->categorySavePath(m_ui->categoryComboBox->currentText());
    m_ui->savePath->addItem(savePath);

    m_ui->downloadPath->blockSignals(true);
    m_ui->downloadPath->clear();
    const Path downloadPath =
        session->categoryDownloadPath(m_ui->categoryComboBox->currentText());
