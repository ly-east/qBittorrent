...
TorrentOptionsDialog::TorrentOptionsDialog(
    QWidget *parent, const QVector<BitTorrent::Torrent *> &torrents)
    : QDialog{parent}, m_ui{new Ui::TorrentOptionsDialog},
      m_storeDialogSize{SETTINGS_KEY(u"Size"_s)},
      m_currentCategoriesString{
          u"--%1--"_s.arg(tr("Currently used categories"))} {
  ...
  const int firstTorrentInactiveSeedingTime =
      torrents[0]->inactiveSeedingTimeLimit();
  ...
  const bool isFirstTorrentSequentialEnabled =
      torrents[0]->isSequentialDownload();
  const bool isFirstTorrentFirstLastPiecesEnabled =
      torrents[0]->hasFirstLastPiecePriority();
  ...
  for (const BitTorrent::Torrent *torrent : torrents) {
    ...
    if (allSameInactiveSeedingTime) {
      if (torrent->inactiveSeedingTimeLimit() !=
          firstTorrentInactiveSeedingTime)
    ...
    if (allSameFirstLastPieces) {
      if (torrent->hasFirstLastPiecePriority() !=
          isFirstTorrentFirstLastPiecesEnabled)
  ...
  if (!m_allSameCategory) {
    ...
    m_ui->comboCategory->lineEdit()->setPlaceholderText(
        m_currentCategoriesString);
  ...
  const int globalUploadLimit =
      isAltLimitEnabled ? (session->altGlobalUploadSpeedLimit() / 1024)
                        : (session->globalUploadSpeedLimit() / 1024);
  const int globalDownloadLimit =
      isAltLimitEnabled ? (session->altGlobalDownloadSpeedLimit() / 1024)
  ...
  // This can happen for example if global rate limit is lower than torrent rate
  // limit.
  ...
  m_ui->sliderUploadLimit->setValue(allSameUpLimit ? uploadVal
                                                   : (maxUpload / 2));
  if (allSameUpLimit) {
  ...
  m_ui->sliderDownloadLimit->setValue(allSameDownLimit ? downloadVal
                                                       : (maxDownload / 2));
  if (allSameDownLimit) {
  ...
  const bool useGlobalValue =
      allSameRatio && allSameSeedingTime &&
  ...
  const int maxSeedingTime =
      (allSameSeedingTime && (firstTorrentSeedingTime >= 0))
          ? firstTorrentSeedingTime
          : session->globalMaxSeedingMinutes();
  const int maxInactiveSeedingTime =
      (allSameInactiveSeedingTime && (firstTorrentInactiveSeedingTime >= 0))
          ? firstTorrentInactiveSeedingTime
  ...
  const QString privateTorrentsTooltip =
  ...
  if (allSameFirstLastPieces)
    m_ui->checkFirstLastPieces->setChecked(
        isFirstTorrentFirstLastPiecesEnabled);
  else
    m_ui->checkFirstLastPieces->setCheckState(Qt::PartiallyChecked);

  m_initialValues = {m_ui->savePath->selectedPath(),
  ...
  connect(m_ui->checkAutoTMM, &QCheckBox::clicked, this,
          &TorrentOptionsDialog::handleTMMChanged);
  connect(m_ui->checkUseDownloadPath, &QCheckBox::clicked, this,
          &TorrentOptionsDialog::handleUseDownloadPathChanged);
  connect(m_ui->comboCategory, &QComboBox::activated, this,
          &TorrentOptionsDialog::handleCategoryChanged);

  // Sync up/down speed limit sliders with their corresponding spinboxes
  connect(m_ui->sliderUploadLimit, &QSlider::valueChanged,
          m_ui->spinUploadLimit, &QSpinBox::setValue);
  connect(m_ui->sliderDownloadLimit, &QSlider::valueChanged,
          m_ui->spinDownloadLimit, &QSpinBox::setValue);
  connect(m_ui->spinUploadLimit, qOverload<int>(&QSpinBox::valueChanged), this,
          [this](const int value) {
            updateSliderValue(m_ui->sliderUploadLimit, value);
          });
  connect(m_ui->spinDownloadLimit, qOverload<int>(&QSpinBox::valueChanged),
  ...
  connect(m_ui->checkMaxRatio, &QCheckBox::toggled, m_ui->spinRatioLimit,
          &QWidget::setEnabled);
  connect(m_ui->checkMaxTime, &QCheckBox::toggled, m_ui->spinTimeLimit,
          &QWidget::setEnabled);
  connect(m_ui->checkMaxInactiveTime, &QCheckBox::toggled,
          m_ui->spinInactiveTimeLimit, &QSpinBox::setEnabled);

  connect(m_ui->buttonGroup, &QButtonGroup::idClicked, this,
          &TorrentOptionsDialog::handleRatioTypeChanged);
...
void TorrentOptionsDialog::accept() {
  if (m_ui->radioTorrentLimit->isChecked() &&
      !m_ui->checkMaxRatio->isChecked() && !m_ui->checkMaxTime->isChecked() &&
      !m_ui->checkMaxInactiveTime->isChecked()) {
  ...
  for (const BitTorrent::TorrentID &id : asConst(m_torrentIDs)) {
    ...
    if ((m_initialValues.category != category) ||
        (m_ui->comboCategory->currentIndex() != 0)) {
    ...
    if (m_initialValues.firstLastPieces !=
        m_ui->checkFirstLastPieces->checkState())
      torrent->setFirstLastPiecePriority(
          m_ui->checkFirstLastPieces->isChecked());
  }
...
int TorrentOptionsDialog::getInactiveSeedingTime() const {
  ...
  if (m_ui->radioNoLimit->isChecked() ||
      !m_ui->checkMaxInactiveTime->isChecked())
...
void TorrentOptionsDialog::handleCategoryChanged(const int index) {
  ...
  if (m_ui->checkAutoTMM->checkState() == Qt::Checked) {
    if (!m_allSameCategory && (m_ui->comboCategory->currentIndex() == 0)) {
      ...
      const Path savePath = BitTorrent::Session::instance()->categorySavePath(
          m_ui->comboCategory->currentText());
      m_ui->savePath->setSelectedPath(savePath);
      const Path downloadPath =
          BitTorrent::Session::instance()->categoryDownloadPath(
              m_ui->comboCategory->currentText());
...
void TorrentOptionsDialog::handleRatioTypeChanged() {
  if ((m_initialValues.ratio == MIXED_SHARE_LIMITS) ||
      (m_initialValues.seedingTime == MIXED_SHARE_LIMITS) ||
    ...
    if (currentRadio && (currentRadio == m_previousRadio)) {
      // Hack to deselect the currently selected radio button programmatically
      // because Qt doesn't allow it in exclusive mode
  ...
  m_ui->spinRatioLimit->setEnabled(m_ui->radioTorrentLimit->isChecked() &&
                                   m_ui->checkMaxRatio->isChecked());
  m_ui->spinTimeLimit->setEnabled(m_ui->radioTorrentLimit->isChecked() &&
                                  m_ui->checkMaxTime->isChecked());
  m_ui->spinInactiveTimeLimit->setEnabled(
