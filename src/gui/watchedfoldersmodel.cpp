...
WatchedFoldersModel::WatchedFoldersModel(TorrentFilesWatcher *fsWatcher,
                                         QObject *parent)
    : QAbstractListModel{parent}, m_fsWatcher{fsWatcher},
      m_watchedFolders{m_fsWatcher->folders().keys()},
      m_watchedFoldersOptions{m_fsWatcher->folders()} {
  connect(m_fsWatcher, &TorrentFilesWatcher::watchedFolderSet, this,
          &WatchedFoldersModel::onFolderSet);
  connect(m_fsWatcher, &TorrentFilesWatcher::watchedFolderRemoved, this,
          &WatchedFoldersModel::onFolderRemoved);
}
...
QVariant WatchedFoldersModel::data(const QModelIndex &index,
...
QVariant WatchedFoldersModel::headerData(const int section,
...
bool WatchedFoldersModel::removeRows(const int row, const int count,
...
void WatchedFoldersModel::addFolder(
    const Path &path,
  ...
  if (m_watchedFoldersOptions.contains(path))
    throw RuntimeError(
        tr("Folder '%1' is already in watch list.").arg(path.toString()));
...
WatchedFoldersModel::folderOptions(const int row) const {
...
void WatchedFoldersModel::setFolderOptions(
    const int row, const TorrentFilesWatcher::WatchedFolderOptions &options) {
  Q_ASSERT((row >= 0) && (row < rowCount()));
...
void WatchedFoldersModel::onFolderSet(
    const Path &path,
    const TorrentFilesWatcher::WatchedFolderOptions &options) {
  if (!m_watchedFoldersOptions.contains(path)) {
    m_deletedFolders.remove(path);
