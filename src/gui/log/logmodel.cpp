...
BaseLogModel::Message::Message(const QString &time, const QString &message,
                               const QColor &foreground,
                               const Log::MsgType type)
    : m_time(time), m_message(message), m_foreground(foreground), m_type(type) {
}

QVariant BaseLogModel::Message::time() const { return m_time; }

QVariant BaseLogModel::Message::message() const { return m_message; }

QVariant BaseLogModel::Message::foreground() const { return m_foreground; }

QVariant BaseLogModel::Message::type() const { return m_type; }

BaseLogModel::BaseLogModel(QObject *parent)
    : QAbstractListModel(parent), m_messages(MAX_VISIBLE_MESSAGES),
...
int BaseLogModel::columnCount(const QModelIndex &) const { return 1; }
...
QVariant BaseLogModel::data(const QModelIndex &index, const int role) const {
  ...
  if ((messageIndex < 0) ||
      (messageIndex >= static_cast<int>(m_messages.size())))
    return {};
...
LogMessageModel::LogMessageModel(QObject *parent)
    : BaseLogModel(parent),
      m_foregroundForMessageTypes{{Log::NORMAL, getLogNormalColor()},
  ...
  connect(Logger::instance(), &Logger::newLogMessage, this,
          &LogMessageModel::handleNewMessage);
}

void LogMessageModel::handleNewMessage(const Log::Msg &message) {
  const QString time = QLocale::system().toString(
      QDateTime::fromSecsSinceEpoch(message.timestamp), QLocale::ShortFormat);
...
LogPeerModel::LogPeerModel(QObject *parent)
  ...
  connect(Logger::instance(), &Logger::newLogPeer, this,
          &LogPeerModel::handleNewMessage);
}

void LogPeerModel::handleNewMessage(const Log::Peer &peer) {
  const QString time = QLocale::system().toString(
      QDateTime::fromSecsSinceEpoch(peer.timestamp), QLocale::ShortFormat);
  const QString message =
      peer.blocked ? tr("%1 was blocked. Reason: %2.",
