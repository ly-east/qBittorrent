...
QString logText(const QModelIndex &index) {
  return index.data(BaseLogModel::TimeRole).toString() + SEPARATOR +
...
private:
  void paint(QPainter *painter, const QStyleOptionViewItem &option,
    ...
    QStyledItemDelegate::paint(
        painter, option,
        index); // paints background, focus rect and selection rect

    const QStyle *style =
        option.widget ? option.widget->style() : QApplication::style();
    const QRect textRect = option.rect.adjusted(
        1, 0, 0, 0); // shift 1 to avoid text being too close to focus rect
    ...
    const QPalette::ColorRole textRole =
        m_useCustomUITheme ? (option.state.testFlag(QStyle::State_Selected)
    ...
    // until they are set explicitly, and we have to manually set some font
    // properties
    QFont font = option.font;
    font.setFamily(option.font.family());
    if (option.font.pointSizeF() >
        0) // for better scaling we use floating point version
    ...
    palette.setColor(
        ...
        index.data(BaseLogModel::TimeForegroundRole).value<QColor>());
    style->drawItemText(painter, textRect, option.displayAlignment, palette,
                        isEnabled, time, textRole);

    const QFontMetrics fontMetrics =
        painter->fontMetrics(); // option.fontMetrics adds extra padding to
                                // QFontMetrics::width
    const int separatorCoordinateX = horizontalAdvance(fontMetrics, time);
    style->drawItemText(painter,
                        textRect.adjusted(separatorCoordinateX, 0, 0, 0),
                        option.displayAlignment, option.palette, isEnabled,
                        SEPARATOR, textRole);

    const int messageCoordinateX =
        separatorCoordinateX + horizontalAdvance(fontMetrics, SEPARATOR);
    const QString message = index.data(BaseLogModel::MessageRole).toString();
    palette.setColor(
        QPalette::Active, QPalette::WindowText,
        index.data(BaseLogModel::MessageForegroundRole).value<QColor>());
    style->drawItemText(painter, textRect.adjusted(messageCoordinateX, 0, 0, 0),
                        option.displayAlignment, palette, isEnabled, message,
                        textRole);
  ...
  QSize sizeHint(const QStyleOptionViewItem &option,
    ...
    const QSize fontSize =
        option.fontMetrics.size(0, logText(index)) + minimumFontPadding;
