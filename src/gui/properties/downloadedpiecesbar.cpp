...
QColor dlPieceColor(const QColor &pieceColor) {
  ...
  return QColor::fromHsl(green.hslHue(), pieceColor.hslSaturation(),
                         pieceColor.lightness());
}
} // namespace
...
QVector<float>
DownloadedPiecesBar::bitfieldToFloatVector(const QBitArray &vecin,
                                           int reqSize) {
  ...
  for (int x = 0; x < reqSize; ++x) {
    ...
    // float precision sometimes gives > 1, because it's not possible to store
    // irrational numbers
...
bool DownloadedPiecesBar::updateImage(QImage &image) {
  ...
  QVector<float> scaledPiecesDl =
      bitfieldToFloatVector(m_downloadedPieces, image2.width());
  ...
  for (int x = 0; x < scaledPieces.size(); ++x) {
    ...
    if (piecesToValueDl != 0) {
      ...
      QRgb mixedColor =
          mixTwoColors(pieceColor().rgb(), m_dlPieceColor.rgb(), ratio);
...
QString DownloadedPiecesBar::simpleToolTipText() const {
  ...
  const QString rowHTML =
      u"<tr><td width=20 bgcolor='%1' style='border: 1px solid \"%2\";'></td><td>%3</td></tr>"_s;
  return u"<table cellspacing=4>" +
         rowHTML.arg(backgroundColor().name(), borderColor,
                     tr("Missing pieces")) +
