...
TrackerListWidget::TrackerListWidget(PropertiesWidget *properties)
  ...
  // Set header
  // Must be set before calling loadSettings() otherwise the header is reset on
  // restart
  ...
  header()->setStretchLastSection(
      false); // Must be set after loadSettings() in order to work
  ...
  connect(this, &QWidget::customContextMenuRequested, this,
          &TrackerListWidget::showTrackerListMenu);
  // Header
  ...
  connect(header(), &QHeaderView::sectionMoved, this,
          &TrackerListWidget::saveSettings);
  ...
  m_DHTItem = new QTreeWidgetItem(
      {u""_s, u"** [DHT] **"_s, u""_s, u"0"_s, u""_s, u""_s, u"0"_s});
  insertTopLevelItem(0, m_DHTItem);
  setRowColor(0, QColorConstants::Svg::grey);
  m_PEXItem = new QTreeWidgetItem(
      {u""_s, u"** [PeX] **"_s, u""_s, u"0"_s, u""_s, u""_s, u"0"_s});
  insertTopLevelItem(1, m_PEXItem);
  setRowColor(1, QColorConstants::Svg::grey);
  m_LSDItem = new QTreeWidgetItem(
      {u""_s, u"** [LSD] **"_s, u""_s, u"0"_s, u""_s, u""_s, u"0"_s});
  ...
  connect(editHotkey, &QShortcut::activated, this,
          &TrackerListWidget::editSelectedTracker);
  ...
  connect(copyHotkey, &QShortcut::activated, this,
          &TrackerListWidget::copyTrackerUrl);

  connect(this, &QAbstractItemView::doubleClicked, this,
          &TrackerListWidget::editSelectedTracker);
}

TrackerListWidget::~TrackerListWidget() { saveSettings(); }
...
void TrackerListWidget::moveSelectionUp() {
  ...
  const QVector<QTreeWidgetItem *> selectedTrackerItems =
      getSelectedTrackerItems();
  if (selectedTrackerItems.isEmpty())
    return;
  ...
  for (QTreeWidgetItem *item : selectedTrackerItems)
    selection->select(indexFromItem(item), (QItemSelectionModel::Rows |
  ...
  for (int i = NB_STICKY_ITEM; i < topLevelItemCount(); ++i) {
    const QString trackerURL =
        topLevelItem(i)->data(COL_URL, Qt::DisplayRole).toString();
...
void TrackerListWidget::loadStickyItems(const BitTorrent::Torrent *torrent) {
  ...
  torrent->fetchPeerInfo([this, torrent = TorrentPtr(torrent)](
    ...
    uint seedsDHT = 0, seedsPeX = 0, seedsLSD = 0, peersDHT = 0, peersPeX = 0,
         peersLSD = 0;
...
void TrackerListWidget::loadTrackers() {
  ...
  for (const BitTorrent::TrackerEntry &entry : asConst(torrent->trackers())) {
    ...
    item->setText(
        COL_PEERS,
        ((entry.numPeers > -1) ? QString::number(entry.numPeers) : tr("N/A")));
    item->setText(
        COL_SEEDS,
        ((entry.numSeeds > -1) ? QString::number(entry.numSeeds) : tr("N/A")));
    item->setText(COL_LEECHES,
                  ((entry.numLeeches > -1) ? QString::number(entry.numLeeches)
                                           : tr("N/A")));
    item->setText(COL_TIMES_DOWNLOADED,
                  ((entry.numDownloaded > -1)
...
void TrackerListWidget::editSelectedTracker() {
  ...
  bool ok = false;
  const QUrl newTrackerURL =
      AutoExpandableDialog::getText(this, tr("Tracker editing"),
                                    tr("Tracker URL:"), QLineEdit::Normal,
                                    trackerURL.toString(), &ok)
          .trimmed();
  if (!ok)
    return;

  if (!newTrackerURL.isValid()) {
    QMessageBox::warning(this, tr("Tracker editing failed"),
                         tr("The tracker URL entered is invalid."));
    return;
  }
  ...
  for (BitTorrent::TrackerEntry &entry : trackers) {
    if (newTrackerURL == QUrl(entry.url)) {
      QMessageBox::warning(this, tr("Tracker editing failed"),
                           tr("The tracker URL already exists."));
      return;
    }
...
void TrackerListWidget::showTrackerListMenu() {
  ...
  menu->addAction(UIThemeManager::instance()->getIcon(u"list-add"_s),
                  tr("Add trackers..."), this,
                  &TrackerListWidget::openAddTrackersDialog);

  if (!getSelectedTrackerItems().isEmpty()) {
    menu->addAction(UIThemeManager::instance()->getIcon(u"edit-rename"_s),
                    tr("Edit tracker URL..."), this,
                    &TrackerListWidget::editSelectedTracker);
    menu->addAction(
        UIThemeManager::instance()->getIcon(u"edit-clear"_s, u"list-remove"_s),
        tr("Remove tracker"), this, &TrackerListWidget::deleteSelectedTrackers);
    menu->addAction(UIThemeManager::instance()->getIcon(u"edit-copy"_s),
                    tr("Copy tracker URL"), this,
                    &TrackerListWidget::copyTrackerUrl);
  }

  if (!torrent->isPaused()) {
    menu->addAction(
        UIThemeManager::instance()->getIcon(u"reannounce"_s, u"view-refresh"_s),
        tr("Force reannounce to selected trackers"), this,
        &TrackerListWidget::reannounceSelected);
    menu->addSeparator();
    menu->addAction(
        UIThemeManager::instance()->getIcon(u"reannounce"_s, u"view-refresh"_s),
        tr("Force reannounce to all trackers"), this, [this]() {
...
QStringList TrackerListWidget::headerLabels() {
  return {tr("Tier"),  tr("URL"),     tr("Status"),           tr("Peers"),
          tr("Seeds"), tr("Leeches"), tr("Times Downloaded"), tr("Message")};
}
...
void TrackerListWidget::displayColumnHeaderMenu() {
  ...
  for (int i = 0; i < COL_COUNT; ++i) {
    QAction *action = menu->addAction(
        headerLabels().at(i), this, [this, i](const bool checked) {
