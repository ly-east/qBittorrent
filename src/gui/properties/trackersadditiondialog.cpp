...
TrackersAdditionDialog::TrackersAdditionDialog(
    QWidget *parent, BitTorrent::Torrent *const torrent)
    : QDialog(parent), m_ui(new Ui::TrackersAdditionDialog), m_torrent(torrent),
  ...
  connect(m_ui->downloadButton, &QAbstractButton::clicked, this,
          &TrackersAdditionDialog::onDownloadButtonClicked);
...
void TrackersAdditionDialog::onAccepted() const {
  const QVector<BitTorrent::TrackerEntry> entries =
      BitTorrent::parseTrackerEntries(
          m_ui->textEditTrackersList->toPlainText());
...
void TrackersAdditionDialog::onDownloadButtonClicked() {
  ...
  if (url.isEmpty()) {
    QMessageBox::warning(this, tr("Trackers list URL error"),
                         tr("The trackers list URL cannot be empty"));
    return;
  }
  ...
  Net::DownloadManager::instance()->download(
      url, Preferences::instance()->useProxyForGeneralPurposes(), this,
      &TrackersAdditionDialog::onTorrentListDownloadFinished);
}

void TrackersAdditionDialog::onTorrentListDownloadFinished(
  ...
  if (result.status != Net::DownloadStatus::Success) {
    QMessageBox::warning(
        this, tr("Download trackers list error"),
        tr("Error occurred when downloading the trackers list. Reason: \"%1\"")
