...
// Computes approximate mapping from image scale (measured in pixels) onto the
// torrent contents scale (in pieces) However, taking the size of a screen to be
// ~ 1000 px and the torrent size larger than 10 MiB, the pointing error is well
// below 0.5 px and thus is negligible.
class PieceIndexToImagePos {
public:
  PieceIndexToImagePos(const BitTorrent::TorrentInfo &torrentInfo,
                       const QImage &image)
      : m_bytesPerPixel{((image.width() > 0) &&
                         (torrentInfo.totalSize() >= image.width()))
                            ? torrentInfo.totalSize() / image.width()
                            : -1},
        m_torrentInfo{torrentInfo} {
    if ((m_bytesPerPixel > 0) && (m_bytesPerPixel < 10))
      qDebug() << "PieceIndexToImagePos: torrent size is too small for correct "
                  "computaions."
  ...
  ImageRange imagePos(const BitTorrent::TorrentInfo::PieceRange &pieces) const {
    ...
    // the type conversion is used to prevent integer overflow with torrents of
    // 2+ GiB size
    ...
    return makeInterval<ImageRange::IndexType>(
        ...
        (pieces.last() * pieceLength +
         m_torrentInfo.pieceLength(pieces.last()) - 1) /
            m_bytesPerPixel);
  }
  ...
  int pieceIndex(int imagePos) const {
    return m_bytesPerPixel < 0
               ? 0
               : (imagePos * m_bytesPerPixel + m_bytesPerPixel / 2) /
                     m_torrentInfo.pieceLength();
  }

private:
  const qlonglong m_bytesPerPixel; // how many bytes of the torrent are squeezed
                                   // into a bar's pixel
...
public:
  ...
  ~DetailedTooltipRenderer() { m_string += u"</table>"; }

  void operator()(const QString &size, const Path &path) {
    m_string += uR"(<tr><td style="white-space:nowrap">)" + size +
                u"</td><td>" + path.toString() + u"</td></tr>";
  }
...
PiecesBar::PiecesBar(QWidget *parent) : QWidget{parent} {
  updatePieceColors();
...
void PiecesBar::paintEvent(QPaintEvent *) {
  ...
  QRect imageRect(borderWidth, borderWidth, width() - 2 * borderWidth,
  ...
  if (!m_highlightedRegion.isNull()) {
    QColor highlightColor{
        this->palette().color(QPalette::Active, QPalette::Highlight)};
    highlightColor.setAlphaF(0.35f);
    QRect targetHighlightRect{m_highlightedRegion.adjusted(
        borderWidth, borderWidth, borderWidth, height() - 2 * borderWidth)};
...
const QVector<QRgb> &PiecesBar::pieceColors() const { return m_pieceColors; }
...
void PiecesBar::showToolTip(const QHelpEvent *e) {
  ...
  const bool showDetailedInformation =
  ...
  if (showDetailedInformation && m_torrent->hasMetadata()) {
    ...
    if ((imagePos >= 0) && (imagePos < m_image.width())) {
      ...
      const QVector<int> fileIndexes =
          torrentInfo.fileIndicesForPiece(pieceIndex);

      QString tooltipTitle;
      if (fileIndexes.count() > 1)
        tooltipTitle = tr("Files in this piece:");
      else if (torrentInfo.fileSize(fileIndexes.front()) ==
               torrentInfo.pieceLength(pieceIndex))
      ...
      for (const int index : fileIndexes) {
        ...
        renderer(Utils::Misc::friendlyUnit(torrentInfo.fileSize(index)),
                 filePath);
      }
    ...
    if (showDetailedInformation) // metadata are not available at this point
      toolTipText += u'\n' + tr("Wait until metadata become available to see "
...
void PiecesBar::highlightFile(int imagePos) {
  if (!m_torrent || !m_torrent->hasMetadata() || (imagePos < 0) ||
      (imagePos >= m_image.width()))
    return;
  ...
  if (fileIndices.count() == 1) {
    BitTorrent::TorrentInfo::PieceRange filePieces =
        torrentInfo.filePieces(fileIndices.first());

    ImageRange imageRange = transform.imagePos(filePieces);
    QRect newHighlightedRegion{imageRange.first(), 0, imageRange.size(),
                               m_image.height()};
