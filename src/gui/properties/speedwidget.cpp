...
SpeedWidget::SpeedWidget(PropertiesWidget *parent) : QWidget(parent) {
...
void SpeedWidget::update() {
  const BitTorrent::SessionStatus &btStatus =
...
void SpeedWidget::onGraphChange(int id) {
  QAction *action = m_graphsMenuActions.at(id);
  m_plot->setGraphEnable(static_cast<SpeedPlotView::GraphID>(id),
                         action->isChecked());
}
