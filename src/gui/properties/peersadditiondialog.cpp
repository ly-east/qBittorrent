...
PeersAdditionDialog::PeersAdditionDialog(QWidget *parent)
  ...
  connect(m_ui->buttonBox, &QDialogButtonBox::accepted, this,
          &PeersAdditionDialog::validateInput);
  connect(m_ui->buttonBox, &QDialogButtonBox::rejected, this, &QDialog::reject);
}

PeersAdditionDialog::~PeersAdditionDialog() { delete m_ui; }

QVector<BitTorrent::PeerAddress>
PeersAdditionDialog::askForPeers(QWidget *parent) {
...
void PeersAdditionDialog::validateInput() {
  if (m_ui->textEditPeers->toPlainText().trimmed().isEmpty()) {
    QMessageBox::warning(this, tr("No peer entered"),
                         tr("Please type at least one peer."), QMessageBox::Ok);
    return;
  }
  for (const QString &peer :
       asConst(m_ui->textEditPeers->toPlainText().trimmed().split(u'\n'))) {
