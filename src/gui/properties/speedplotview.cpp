...
namespace {
// table of supposed nice steps for grid marks to get nice looking quarters of
// scale
...
struct SplitValue {
  ...
  qint64 sizeInBytes() const { return Utils::Misc::sizeInBytes(arg, unit); }
};
...
SplitValue getRoundedYScale(double value) {
  ...
  while (value > 1024) {
    value /= 1024;
    calculatedUnit =
        static_cast<SizeUnit>(static_cast<int>(calculatedUnit) + 1);
  }
...
QString formatLabel(const double argValue, const Utils::Misc::SizeUnit unit) {
  ...
  return QLocale::system().toString(argValue, 'f', precision) +
         C_NON_BREAKING_SPACE + unitString(unit, true);
}
} // namespace

SpeedPlotView::Averager::Averager(const milliseconds duration,
                                  const milliseconds resolution)
    : m_resolution{resolution}, m_maxDuration{duration},
      m_sink{
          static_cast<DataCircularBuffer::size_type>(duration / resolution)} {
  m_lastSampleTime.start();
}

bool SpeedPlotView::Averager::push(const SampleData &sampleData) {
  // Accumulator overflow will be hit in worst case on longest used averaging
  // span, defined by resolution. Maximum resolution is 144 seconds Using int32
  // for accumulator we get overflow when transfer speed reaches 2^31/144
  // ~~ 14.2 MBytes/s. With quint64 this speed limit is 2^64/144 ~~ 114
  // PBytes/s. This speed is inaccessible to an ordinary user.
  ++m_counter;
  for (int id = UP; id < NB_GRAPHS; ++id)
    m_accumulator[id] += sampleData[id];

  // system may go to sleep, that can cause very big elapsed interval
  const milliseconds updateInterval{static_cast<int64_t>(
      BitTorrent::Session::instance()->refreshInterval() * 1.25)};
  const milliseconds maxElapsed{std::max(updateInterval, m_resolution)};
  const milliseconds elapsed{
      std::min(milliseconds{m_lastSampleTime.elapsed()}, maxElapsed)};
...
SpeedPlotView::SpeedPlotView(QWidget *parent) : QGraphicsView{parent} {
  QPen greenPen;
...
void SpeedPlotView::pushPoint(const SpeedPlotView::SampleData &point) {
  for (Averager *averager :
       {&m_averager5Min, &m_averager30Min, &m_averager6Hour, &m_averager12Hour,
        &m_averager24Hour}) {
...
void SpeedPlotView::paintEvent(QPaintEvent *) {
  ...
  rect.adjust(0, fontMetrics.height(), 0,
              0); // Add top padding for top speed text
  ...
  int i = 0;
  for (const QString &label : speedLabels) {
    QRectF labelRect(rect.topLeft() +
                         QPointF(-yAxisWidth, (i++) * 0.25 * rect.height() -
                                                  fontMetrics.height()),
  ...
  painter.drawLine(fullRect.left(), rect.top() + 0.25 * rect.height(),
                   rect.right(), rect.top() + 0.25 * rect.height());
  painter.drawLine(fullRect.left(), rect.top() + 0.50 * rect.height(),
                   rect.right(), rect.top() + 0.50 * rect.height());
  painter.drawLine(fullRect.left(), rect.top() + 0.75 * rect.height(),
                   rect.right(), rect.top() + 0.75 * rect.height());
  ...
  // last point will be drawn at x=0, so we don't need it in the calculation of
  // xTickSize
  const milliseconds lastDuration{queue.empty() ? 0ms : queue.back().duration};
  const double xTickSize = static_cast<double>(rect.width()) /
                           (m_currentMaxDuration - lastDuration).count();
  const double yMultiplier =
      (niceScale.arg == 0)
          ? 0
          : (static_cast<double>(rect.height()) / niceScale.sizeInBytes());
  ...
  QRectF legendBackgroundRect(
      QPoint(legendTopLeft.x() - 4, legendTopLeft.y() - 4),
      QSizeF(legendWidth + 8, legendHeight + 8));
  QColor legendBackgroundColor =
  ...
  for (const auto &property : asConst(m_properties)) {
    ...
    painter.drawLine(legendTopLeft + QPointF(0, indent + fontMetrics.height()),
                     legendTopLeft +
                         QPointF(nameSize, indent + fontMetrics.height()));
    painter.drawText(QRectF(legendTopLeft + QPointF(0, indent),
                            QSizeF(2 * nameSize, fontMetrics.height())),
                     property.name, QTextOption(Qt::AlignVCenter));
  }
}

SpeedPlotView::GraphProperties::GraphProperties() : enable(false) {}

SpeedPlotView::GraphProperties::GraphProperties(const QString &name,
                                                const QPen &pen, bool enable)
