...
bool TorrentContentModelItem::isRootItem() const { return !m_parentItem; }
...
qulonglong TorrentContentModelItem::remaining() const {
  Q_ASSERT(!isRootItem());
  return (m_priority == BitTorrent::DownloadPriority::Ignored) ? 0
                                                               : m_remaining;
}
...
int TorrentContentModelItem::columnCount() const { return NB_COL; }
...
QString TorrentContentModelItem::displayData(const int column) const {
  ...
  case COL_AVAILABILITY: {
    ...
    const QString value =
        (avail >= 1) ? u"100"_s : Utils::String::fromDouble((avail * 100), 1);
...
int TorrentContentModelItem::row() const {
  if (m_parentItem)
    return m_parentItem->children().indexOf(
        const_cast<TorrentContentModelItem *>(this));
  return 0;
}
