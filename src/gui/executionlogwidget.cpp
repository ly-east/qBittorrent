...
ExecutionLogWidget::ExecutionLogWidget(const Log::MsgTypes types,
                                       QWidget *parent)
    : QWidget(parent), m_ui(new Ui::ExecutionLogWidget),
  ...
  connect(messageView, &LogListView::customContextMenuRequested, this,
          [this, messageView, messageModel]() {
  ...
  connect(peerView, &LogListView::customContextMenuRequested, this,
          [this, peerView, peerModel]() {
...
#ifndef Q_OS_MACOS
  m_ui->tabConsole->setTabIcon(
      0, UIThemeManager::instance()->getIcon(u"help-contents"_s,
                                             u"view-calendar-journal"_s));
  m_ui->tabConsole->setTabIcon(1, UIThemeManager::instance()->getIcon(
                                      u"ip-blocked"_s, u"view-filter"_s));
#endif
}

ExecutionLogWidget::~ExecutionLogWidget() { delete m_ui; }

void ExecutionLogWidget::setMessageTypes(const Log::MsgTypes types) {
  m_messageFilterModel->setMessageTypes(types);
}

void ExecutionLogWidget::displayContextMenu(const LogListView *view,
                                            const BaseLogModel *model) const {
  ...
  if (view->currentIndex().isValid()) {
    menu->addAction(UIThemeManager::instance()->getIcon(u"edit-copy"_s),
                    tr("Copy"), view, &LogListView::copySelection);
  }

  menu->addAction(UIThemeManager::instance()->getIcon(u"edit-clear"_s),
                  tr("Clear"), model, &BaseLogModel::reset);
